 ...
 */
package com.codefollower.lealone.hbase.type;
...
import com.codefollower.lealone.value.ValueResultSet;
    ...
    private ByteBuffer writeValue(ByteBuffer buff, Value v) {
        ...
        case Value.RESULT_SET: {
            ...
            try {
                ...
                while (rs.next()) {
                    buff.put((byte) 1);
                    for (int i = 0; i < columnCount; i++) {
                        int t = com.codefollower.lealone.value.DataType.convertSQLTypeToValueType(meta.getColumnType(i + 1));
                        Value val = com.codefollower.lealone.value.DataType.readValue(null, rs, i + 1, t);
        ...
        if (SysProperties.CHECK2) {
            if (buff.position() - start != Data.getValueLen(v, handler)) {
                throw DbException.throwInternalError("value size error: got " + (buff.position() - start) + " expected "
