...
public class TransactionStore {
    ...
    public synchronized Transaction begin() {
        ...
        if (transactionId > maxTransactionId) {
            throw DataUtils.newIllegalStateException(DataUtils.ERROR_TOO_MANY_OPEN_TRANSACTIONS,
                    "There are {0} open transactions", transactionId - 1);
        }
        openTransactions.set(transactionId);
        status = Transaction.STATUS_OPEN;
        ...
        VersionedValue getValue(K key, long maxLog, VersionedValue data) {
            ...
            if (MVStore.ASSERT) {
                if (!Thread.holdsLock(getUndoLog())) {
                    throw DataUtils.newIllegalStateException(DataUtils.ERROR_INTERNAL, "not synchronized on undoLog");
                }
            }
            while (true) {
                if (data == null) {
    ...
    static class VersionedValue {
        ...
        public String toString() {
            return value
                    + (operationId == 0 ? "" : (" " + getTransactionId(operationId) + "/" + getLogId(operationId)));
        }
