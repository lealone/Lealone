...
public class CipherFactory {
    ...
    /**
     * The default password to use for the .lealone.keystore file
     */
    private static final String KEYSTORE_PASSWORD = "lealone";
    private static final String KEYSTORE = "~/.lealone.keystore";
    ...
    public static Socket createSocket(InetAddress address, int port) throws IOException {
        // if (address != null)
        // return SSLFactory.getSocket(new ClientEncryptionOptions(), address, port);
    ...
    public static ServerSocket createServerSocket(String listenAddress, int port) throws IOException {
        ...
        // return SSLFactory
        // .getServerSocket(new ServerEncryptionOptions(), InetAddress.getByName(listenAddress), port);
        setKeystore();
        ...
        ServerSocket socket = secureSocket;
    ...
    private static void setKeystore() throws IOException {
        ...
        if (p.getProperty(KEYSTORE_KEY) == null) {
            ...
            try {
                getKeyStore(KEYSTORE_PASSWORD).store(bout, KEYSTORE_PASSWORD.toCharArray());
            } catch (Exception e) {
     ...
     * @return the keystore
     */
    private static KeyStore getKeyStore(String password) throws IOException {
        try {
            ...
            Certificate[] certs = { CertificateFactory.getInstance("X.509").generateCertificate(
                    new ByteArrayInputStream(StringUtils.convertHexToBytes("3082018b3081f502044295ce6b300d06092a8648"
                            + "86f70d0101040500300d310b3009060355040313" + "024832301e170d3035303532363133323630335a"
                            + "170d3337303933303036353734375a300d310b30" + "0906035504031302483230819f300d06092a8648"
                            + "86f70d010101050003818d0030818902818100dc" + "0a13c602b7141110eade2f051b54777b060d0f74"
                            + "e6a110f9cce81159f271ebc88d8e8aa1f743b505" + "fc2e7dfe38d33b8d3f64d1b363d1af4d87783389"
                            + "7954cbaec2fa384c22a415498cf306bb07ac09b7" + "6b001cd68bf77ea0a628f5101959cf2993a9c23d"
                            + "bee79b19305977f8715ae78d023471194cc900b2" + "31eecb0aaea98d0203010001300d06092a864886"
                            + "f70d01010405000381810083f4401a279453701b" + "ef9a7681a5b8b24f153f7d18c7c892133d97bd5f"
                            + "13736be7505290a445a7d5ceb75522403e509751" + "5cd966ded6351ff60d5193de34cd36e5cb04d380"
                            + "398e66286f99923fd92296645fd4ada45844d194" + "dfd815e6cd57f385c117be982809028bba1116c8"
                            + "5740b3d27a55b1a0948bf291ddba44bed337b9"))), };
            store.setKeyEntry("lealone", privateKey, passwordArray, certs);
            // --- generated code end ---
            return store;
        } catch (Exception e) {
            throw DbException.convertToIOException(e);
