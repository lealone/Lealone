...
public class SHA256 {
    ...
    private static final int[] HH = { 0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f,
    ...
    private void calculateHMAC(byte[] key, byte[] message, int len, byte[] iKey, byte[] oKey,
    ...
    private void calculateHash(byte[] data, int len, byte[] byteBuff, int[] intBuff) {
        ...
        for (int block = 0; block < intLen; block += 16) {
            ...
            for (int i = 0; i < 64; i++) {
                int t1 = h + (rot(e, 6) ^ rot(e, 11) ^ rot(e, 25)) + ((e & f) ^ ((~e) & g)) + K[i]
                        + w[i];
