...
public class FileEncrypt extends FileBase {
    ...
    private static final byte[] HEADER = "LealoneEncrypt\n".getBytes();
    ...
    public FileEncrypt(String name, byte[] encryptionKey, FileChannel base) {
        this(name, null, encryptionKey, base);
    }

    public FileEncrypt(String name, String cipher, byte[] encryptionKey, FileChannel base) {
    ...
    private void init() throws IOException {
        ...
        if (newFile) {
            ...
            if (SysProperties.CHECK) {
                byte[] header = new byte[SALT_POS];
                DataUtils.readFully(base, 0, ByteBuffer.wrap(header));
        ...
        BlockCipher cipher;
        if (this.cipher != null)
            cipher = CipherFactory.getBlockCipher(this.cipher);
        else
            cipher = new AES();
