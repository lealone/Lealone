 ...
 */
package com.codefollower.h2.message;
...
public class DbException extends RuntimeException {
    ...
    static {
        try {
            byte[] messages = Utils.getResource("/com/codefollower/h2/res/_messages_en.prop");
            if (messages != null) {
                MESSAGES.load(new ByteArrayInputStream(messages));
            }
            String language = Locale.getDefault().getLanguage();
            if (!"en".equals(language)) {
                byte[] translations = Utils.getResource("/com/codefollower/h2/res/_messages_" + language + ".prop");
