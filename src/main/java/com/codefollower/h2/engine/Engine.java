...
public class Engine implements SessionFactory {
    ...
    private Session openSession(ConnectionInfo ci, boolean ifExists, String cipher) {
        ...
        synchronized (database) {
            ...
            if (ci.getProperty("JMX", false)) {
                try {
                    Utils.callStaticMethod("com.codefollower.h2.jmx.DatabaseInfo.registerMBean", ci, database);
                } catch (Exception e) {
    ...
    void close(String name) {
        if (jmx) {
            try {
                Utils.callStaticMethod("com.codefollower.h2.jmx.DatabaseInfo.unregisterMBean", name);
            } catch (Exception e) {
                throw DbException.get(ErrorCode.FEATURE_NOT_SUPPORTED_1, e, "JMX");
            }
        }
