...
import org.lealone.cbase.mvstore.type.ObjectDataType;
...
public class TransactionStore {
     ...
     */
    public List<CBaseTransaction> getOpenTransactions() {
        synchronized (undoLog) {
            ArrayList<CBaseTransaction> list = New.arrayList();
            Long key = undoLog.firstKey();
            while (key != null) {
                ...
                if (data == null) {
                    if (undoLog.containsKey(getOperationId(transactionId, 0))) {
                        status = CBaseTransaction.STATUS_OPEN;
                    } else {
                        status = CBaseTransaction.STATUS_COMMITTING;
                    }
                    name = null;
                } else {
                ...
                CBaseTransaction t = new CBaseTransaction(this, transactionId, status, name, logId);
                list.add(t);
    ...
    public synchronized CBaseTransaction begin() {
        ...
        int status = CBaseTransaction.STATUS_OPEN;
        return new CBaseTransaction(this, transactionId, status, null, 0);
    }
     ...
     */
    synchronized void storeTransaction(CBaseTransaction t) {
        if (t.getStatus() == CBaseTransaction.STATUS_PREPARED || t.getName() != null) {
     ...
     */
    void log(CBaseTransaction t, long logId, int mapId, Object key, Object oldValue) {
     ...
     * @param logId the log id
     */
    public void logUndo(CBaseTransaction t, long logId) {
     ...
     */
    void commit(CBaseTransaction t, long maxLogId) {
        if (store.isClosed()) {
            return;
        }
        // TODO could synchronize on blocks (100 at a time or so)
        synchronized (undoLog) {
            t.setStatus(CBaseTransaction.STATUS_COMMITTING);
     ...
     * @param t the transaction
     */
    synchronized void endTransaction(CBaseTransaction t) {
        if (t.getStatus() == CBaseTransaction.STATUS_PREPARED) {
            preparedTransactions.remove(t.getId());
        }
        t.setStatus(CBaseTransaction.STATUS_CLOSED);
     ...
     */
    void rollbackTo(CBaseTransaction t, long maxLogId, long toLogId) {
        // TODO could synchronize on blocks (100 at a time or so)
        synchronized (undoLog) {
     ...
     * @return the changes
     */
    Iterator<Change> getChanges(final CBaseTransaction t, final long maxLogId, final long toLogId) {
