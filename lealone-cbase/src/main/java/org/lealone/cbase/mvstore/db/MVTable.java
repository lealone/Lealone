...
import org.lealone.cbase.mvstore.db.MVTableEngine.Store;
import org.lealone.cbase.mvstore.db.TransactionStore.Transaction;
...
public class MVTable extends TableBase {
    ...
    void init(Session session) {
        primaryIndex = new MVPrimaryIndex(session.getDatabase(), this, getId(), IndexColumn.wrap(getColumns()),
    ...
    private void rebuildIndex(Session session, MVIndex index, String indexName) {
        try {
            ...
            if (MVTableEngine.getStore(session) == null) {
                // in-memory
    ...
    private void rebuildIndexBlockMerge(Session session, MVIndex index) {
        ...
        long i = 0;
        Store store = MVTableEngine.getStore(session);
    ...
    public void removeChildrenAndResources(Session session) {
        ...
        MVTableEngine.getStore(session).removeTable(this);
