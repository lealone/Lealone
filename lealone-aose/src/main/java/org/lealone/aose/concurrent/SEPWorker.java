...
final class SEPWorker extends AtomicReference<SEPWorker.Work> implements Runnable {
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                // if stop was signalled, go to sleep (don't try self-assign; being put to sleep is rare, so let's obey
                // it
                // whenever we receive it - though we don't apply this constraint to producers, who may reschedule us
                // before
                // we go to sleep)
                if (stop())
                    while (isStopped())
    ...
    // exit the spinning state; if there are no remaining spinners, we immediately try and schedule work for all
    // executors
