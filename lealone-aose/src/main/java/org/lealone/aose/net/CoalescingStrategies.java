...
public class CoalescingStrategies {
     ...
     */
    private static final String DEBUG_COALESCING_PROPERTY = Constants.PROJECT_NAME_PREFIX + "coalescing_debug";
    private static final boolean DEBUG_COALESCING = Boolean.getBoolean(DEBUG_COALESCING_PROPERTY);

    private static final String DEBUG_COALESCING_PATH_PROPERTY = Constants.PROJECT_NAME_PREFIX
            + "coalescing_debug_path";
    private static final String DEBUG_COALESCING_PATH = System.getProperty(DEBUG_COALESCING_PATH_PROPERTY,
            "/tmp/coleascing_debug");
    ...
    public static abstract class CoalescingStrategy {
        ...
        protected CoalescingStrategy(Parker parker, Logger logger, String displayName) {
            ...
            if (DEBUG_COALESCING) {
                try {
                    File outFile = File.createTempFile("coalescing_" + this.displayName + "_", ".log",
                            new File(DEBUG_COALESCING_PATH));
                    ras = rasTemp = new RandomAccessFile(outFile, "rw");
         ...
         */
        public <C extends Coalescable> void coalesce(BlockingQueue<C> input, List<C> out, int maxItems)
                throws InterruptedException {
            Preconditions.checkArgument(out.isEmpty(), "out list should be empty");
            coalesceInternal(input, out, maxItems);
        }

        protected abstract <C extends Coalescable> void coalesceInternal(BlockingQueue<C> input, List<C> out,
                int maxItems) throws InterruptedException;
    ...
    static class TimeHorizonMovingAverageCoalescingStrategy extends CoalescingStrategy {
        ...
        public TimeHorizonMovingAverageCoalescingStrategy(int maxCoalesceWindow, Parker parker, Logger logger,
                String displayName) {
        ...
        @Override
        protected <C extends Coalescable> void coalesceInternal(BlockingQueue<C> input, List<C> out, int maxItems)
                throws InterruptedException {
    ...
    static class MovingAverageCoalescingStrategy extends CoalescingStrategy {
        ...
        public MovingAverageCoalescingStrategy(int maxCoalesceWindow, Parker parker, Logger logger,
                String displayName) {
            super(parker, logger, displayName);
            this.maxCoalesceWindow = TimeUnit.MICROSECONDS.toNanos(maxCoalesceWindow);
    ...
    // @VisibleForTesting
    static CoalescingStrategy newCoalescingStrategy(String strategy, int coalesceWindow, Parker parker, Logger logger,
            String displayName) {
    ...
    public static CoalescingStrategy newCoalescingStrategy(String strategy, int coalesceWindow, Logger logger,
            String displayName) {
