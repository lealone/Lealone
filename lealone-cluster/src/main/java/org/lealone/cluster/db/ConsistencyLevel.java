...
import org.lealone.cluster.config.ReadRepairDecision;
import org.lealone.cluster.exceptions.InvalidRequestException;
import org.lealone.cluster.exceptions.ProtocolException;
...
public enum ConsistencyLevel {
    ...
    private int localQuorumFor(Keyspace keyspace, String dc) {
        return (keyspace.getReplicationStrategy() instanceof NetworkTopologyStrategy) ? (((NetworkTopologyStrategy) keyspace
                .getReplicationStrategy()).getReplicationFactor(dc) / 2) + 1 : quorumFor(keyspace);
    }
        ...
        case LOCAL_QUORUM:
            ...
            if (localLive < blockFor) {
                if (logger.isDebugEnabled()) {
                    ...
                    builder.append("] are insufficient to satisfy LOCAL_QUORUM requirement of ").append(blockFor)
                            .append(" live nodes in '").append(DatabaseDescriptor.getLocalDataCenter()).append("'");
