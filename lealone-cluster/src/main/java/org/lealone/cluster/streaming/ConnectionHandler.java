...
import org.lealone.cluster.streaming.messages.StreamInitMessage;
 ...
 * {@link StreamSession#messageReceived(org.lealone.cluster.streaming.messages.StreamMessage)}.
 */
public class ConnectionHandler {
    ...
    private final IncomingMessageHandler incoming;
    private final OutgoingMessageHandler outgoing;
     ...
     *
     * @param socket socket to use for {@link org.lealone.cluster.streaming.ConnectionHandler.OutgoingMessageHandler}.
    ...
    abstract static class MessageHandler implements Runnable {
        ...
        protected static DataOutputStreamPlus getWriteChannel(Socket socket) throws IOException {
            return new DataOutputStreamPlus(new BufferedOutputStream(socket.getOutputStream()));
            // WritableByteChannel out = socket.getChannel();
            // // socket channel is null when encrypted(SSL)
            // if (out == null)
            // return new DataOutputStreamPlus(new BufferedOutputStream(socket.getOutputStream()));
            // return new DataOutputStreamPlus(out);
        }
        ...
        public void sendInitMessage(Socket socket, boolean isForOutgoing) throws IOException {
            StreamInitMessage message = new StreamInitMessage(Utils.getBroadcastAddress(), session.sessionIndex(),
    ...
    static class OutgoingMessageHandler extends MessageHandler {
         ...
         */
        private final PriorityBlockingQueue<StreamMessage> messageQueue = new PriorityBlockingQueue<>(64,
