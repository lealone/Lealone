...
/**
 * StreamInitMessage is first sent from the node where {@link org.lealone.cluster.streaming.StreamSession} is started,
 * to initiate corresponding {@link org.lealone.cluster.streaming.StreamSession} on the other side.
 */
public class StreamInitMessage {
    ...
    public ByteBuffer createMessage(boolean compress, int version) {
        ...
        try {
            ...
            ByteArrayOutputStream a = new ByteArrayOutputStream(size);
            try (DataOutputStreamPlus buffer = new DataOutputStreamPlus(a)) {
