...
public class ReplicationFinishedVerbHandler implements IVerbHandler<Object> {
    private static final Logger logger = LoggerFactory.getLogger(ReplicationFinishedVerbHandler.class);

    @Override
    public void doVerb(MessageIn<Object> msg, int id) {
        StorageService.instance.confirmReplication(msg.from);
        MessageOut<Object> response = new MessageOut<>(MessagingService.Verb.INTERNAL_RESPONSE);
