...
import com.codefollower.lealone.engine.RegularDatabaseEngine;
...
import com.codefollower.lealone.store.fs.Recorder;
...
public class RecoverTester implements Recorder {
    ...
    private int writeCount = Utils.getProperty("lealone.recoverTestOffset", 0);
    private int testEvery = Utils.getProperty("lealone.recoverTest", 64);
    private final long maxFileSize = Utils.getProperty("lealone.recoverTestMaxFileSize", Integer.MAX_VALUE) * 1024L * 1024;
    ...
    private synchronized void testDatabase(String fileName, PrintWriter out) {
        ...
        try {
            ...
            ConnectionInfo ci = new ConnectionInfo("jdbc:lealone:" + testDatabase + ";FILE_LOCK=NO;TRACE_LEVEL_FILE=0", p);
        ...
        try {
            ...
            // avoid using the Engine class to avoid deadlocks
            Properties p = new Properties();
            ConnectionInfo ci = new ConnectionInfo("jdbc:lealone:" + testDatabase + ";FILE_LOCK=NO", p);
            Database database = RegularDatabaseEngine.getInstance().createDatabase();
            database.init(ci, null);
            // close the database
            database.removeSession(null);
