...
import com.lealone.server.http.protocol.util.parser.Authorization;
...
public class DigestAuthenticator extends AuthenticatorBase {
    ...
    public void setAlgorithms(String algorithmsString) {
        ...
        for (String algorithm : algorithmsArray) {
            ...
            if (authDigest == null) {
                log.warn(sm.getString("digestAuthenticator.invalidAlgorithm", algorithmsString,
                        algorithm));
                return;
            }
        ...
        if (authorization != null) {
            if (digestInfo.parse(request, authorization)) {
                ...
                if (principal != null && !digestInfo.isNonceStale()) {
                    register(request, response, principal, HttpServletRequest.DIGEST_AUTH,
    ...
    protected void setAuthenticateHeader(HttpServletRequest request, HttpServletResponse response,
            String nonce, boolean isNonceStale) {
    ...
    protected boolean isPreemptiveAuthPossible(Request request) {
        MessageBytes authorizationHeader = request.getCoyoteRequest().getMimeHeaders()
    ...
    public static class DigestInfo {
        ...
        public Principal authenticate(Realm realm) {
            ...
            byte[] buffer = ConcurrentMessageDigest.digest(algorithm.getJavaName(),
