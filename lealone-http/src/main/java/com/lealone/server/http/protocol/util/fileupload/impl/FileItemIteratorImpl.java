 ...
 */
package com.lealone.server.http.protocol.util.fileupload.impl;
...
import com.lealone.server.http.protocol.util.fileupload.FileItemHeaders;
import com.lealone.server.http.protocol.util.fileupload.FileItemIterator;
import com.lealone.server.http.protocol.util.fileupload.FileItemStream;
import com.lealone.server.http.protocol.util.fileupload.FileUploadBase;
import com.lealone.server.http.protocol.util.fileupload.FileUploadException;
import com.lealone.server.http.protocol.util.fileupload.IOUtils;
import com.lealone.server.http.protocol.util.fileupload.MultipartStream;
import com.lealone.server.http.protocol.util.fileupload.ProgressListener;
import com.lealone.server.http.protocol.util.fileupload.RequestContext;
import com.lealone.server.http.protocol.util.fileupload.UploadContext;
import com.lealone.server.http.protocol.util.fileupload.util.LimitedInputStream;
...
public class FileItemIteratorImpl implements FileItemIterator {
    ...
    protected void init(final FileUploadBase fileUploadBase, final RequestContext pRequestContext)
        ...
        if (sizeMax >= 0) {
            ...
            input = new LimitedInputStream(ctx.getInputStream(), sizeMax) {
                ...
                protected void raiseError(final long pSizeMax, final long pCount) throws IOException {
                    final FileUploadException ex = new SizeLimitExceededException(String.format(
                            "the request was rejected because its size (%s) exceeds the configured maximum (%s)",
        ...
        progressNotifier = new MultipartStream.ProgressNotifier(fileUploadBase.getProgressListener(),
                requestSize);
        try {
            ...
            throw new InvalidContentTypeException(
                    String.format("The boundary specified in the %s header is too long",
        ...
        for (;;) {
            ...
            if (currentFieldName == null) {
                ...
                if (fieldName != null) {
                    ...
                    if (subContentType != null && subContentType.toLowerCase(Locale.ENGLISH)
                    ...
                    currentItem = new FileItemStreamImpl(this, fileName, fieldName,
                            headers.getHeader(FileUploadBase.CONTENT_TYPE), fileName == null,
                ...
                final String fileName = fileUploadBase.getFileName(headers);
                if (fileName != null) {
                    currentItem = new FileItemStreamImpl(this, fileName, currentFieldName,
                            headers.getHeader(FileUploadBase.CONTENT_TYPE), false,
                            getContentLength(headers));
                    currentItem.setHeaders(headers);
                    progressNotifier.noteItem();
                    itemValid = true;
