...
import com.lealone.server.http.util.net.openssl.ciphers.OpenSSLCipherConfigurationParser;
...
public class SSLHostConfig implements Serializable {
    ...
    boolean setProperty(String name, Type configType) {
        if (this.configType == null) {
            ...
            if (configType != this.configType) {
                log.warn(sm.getString("sslHostConfig.mismatch", name, getHostName(), configType,
                        this.configType));
                return false;
            }
        }
    ...
    boolean setTrustProperty(String name, Type trustConfigType) {
        if (this.trustConfigType == null) {
            ...
            if (trustConfigType != this.trustConfigType) {
                log.warn(sm.getString("sslHostConfig.mismatch.trust", name, getHostName(),
                        trustConfigType, this.trustConfigType));
                return false;
            }
        }
        return true;
    ...
    private void registerDefaultCertificate() {
        if (defaultCertificate == null) {
            SSLHostConfigCertificate defaultCertificate = new SSLHostConfigCertificate(this,
    ...
    public enum CertificateVerification {
        ...
        public static CertificateVerification fromString(String value) {
            if ("true".equalsIgnoreCase(value) || "yes".equalsIgnoreCase(value)
                    || "require".equalsIgnoreCase(value) || "required".equalsIgnoreCase(value)) {
                return REQUIRED;
