...
public class StructuredField {
    ...
    static SfItem<?> parseSfBareItem(Reader input) throws IOException {
        ...
        if (c == '-' || HttpParser.isNumeric(c)) {
            ...
            throw new IllegalArgumentException(sm.getString("sf.bareitem.invalidCharacter",
    ...
    static String parseSfKey(Reader input) throws IOException {
        ...
        int c = input.read();
        if (!isKeyFirst(c)) {
            throw new IllegalArgumentException(sm.getString("sf.key.invalidFirstCharacter",
                    String.format("\\u%04X", Integer.valueOf(c))));
        }
    ...
    static SfItem<?> parseSfNumeric(Reader input, int first) throws IOException {
        ...
        if (!HttpParser.isNumeric(c)) {
            throw new IllegalArgumentException(sm.getString("sf.numeric.invalidCharacter",
                    String.format("\\u%04X", Integer.valueOf(c))));
        }
        result.append((char) c);
        input.mark(1);
    ...
    static SfString parseSfString(Reader input) throws IOException {
        ...
        while (true) {
            ...
            if (c == '\\') {
                ...
                c = input.read();
                if (c != '\\' && c != '\"') {
                    throw new IllegalArgumentException(sm.getString("sf.string.invalidEscape",
                            String.format("\\u%04X", Integer.valueOf(c))));
                }
            } else {
                if (c == '\"') {
                ...
                if (c < 32 || c > 126) {
                    throw new IllegalArgumentException(sm.getString("sf.string.invalidCharacter",
                            String.format("\\u%04X", Integer.valueOf(c))));
                }
            }
            result.append((char) c);
    ...
    static SfByteSequence parseSfByteSequence(Reader input) throws IOException {
        ...
        while (true) {
            ...
            if (c == ':') {
                ...
                throw new IllegalArgumentException(sm.getString("sf.base64.invalidCharacter",
                        String.format("\\u%04X", Integer.valueOf(c))));
            }
        }
    ...
    static SfBoolean parseSfBoolean(Reader input) throws IOException {
        ...
        if (c == '1') {
            ...
            throw new IllegalArgumentException(sm.getString("sf.boolean.invalidCharacter",
                    String.format("\\u%04X", Integer.valueOf(c))));
        }
    }
