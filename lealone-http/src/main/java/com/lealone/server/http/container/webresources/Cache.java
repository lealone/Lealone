...
import com.lealone.server.http.container.WebResourceRoot.CacheStrategy;
...
public class Cache {
        ...
        if (cacheEntry == null) {
            ...
            if (cacheEntry == null) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug(sm.getString("cache.sizeTracking.add", Long.toString(delta), cacheEntry,
                            path, Long.toString(result)));
                }
    ...
    protected void backgroundProcess() {
        ...
        if (newSize > targetSize) {
            log.info(sm.getString("cache.backgroundEvictFail",
                    Long.valueOf(TARGET_FREE_PERCENT_BACKGROUND), root.getContext().getName(),
    ...
    void removeCacheEntry(String path) {
        ...
        if (cachedResource != null) {
            ...
            long result = size.addAndGet(-delta);
            if (log.isDebugEnabled()) {
                log.debug(sm.getString("cache.sizeTracking.remove", Long.toString(delta), cachedResource,
                        path, Long.toString(result)));
            }
        }
    }
