 ...
 */
package com.lealone.server.http.util.net.jsse;
...
import com.lealone.server.http.util.buf.Asn1Parser;
import com.lealone.server.http.util.buf.Asn1Writer;
...
public class PEMFile {
    ...
    private static final byte[] OID_EC_PUBLIC_KEY = new byte[] {
            0x06,
            0x07,
            0x2A,
            (byte) 0x86,
    ...
    private static final byte[] OID_PBES2 = new byte[] {
            0x2A,
            (byte) 0x86,
            0x48,
            (byte) 0x86,
    ...
    private static final byte[] OID_PBKDF2 = new byte[] {
            0x2A,
            (byte) 0x86,
            0x48,
            (byte) 0x86,
    ...
    public PEMFile(String filename, InputStream fileStream, String password, String passwordFilename,
        ...
        try (BufferedReader reader = new BufferedReader(
                new InputStreamReader(fileStream, StandardCharsets.US_ASCII))) {
            Part part = null;
            String line;
        ...
        if (passwordFileStream != null) {
            try (BufferedReader reader = new BufferedReader(
                    new InputStreamReader(passwordFileStream, StandardCharsets.UTF_8))) {
    ...
    private static class Part {
        ...
        public PrivateKey toPrivateKey(String password, String keyAlgorithm, Format format,
            ...
            default: {
                ...
                throw new NoSuchAlgorithmException(
                        sm.getString("pemFile.unknownEncryptedFormat", format));
            }
            }
        }

        private PrivateKey toPrivateKey(String keyAlgorithm, Format format, String filename,
                byte[] source) throws GeneralSecurityException {
            ...
            InvalidKeyException exception = new InvalidKeyException(
                    sm.getString("pemFile.parseError", filename));
        ...
        private byte[] deriveKeyPBKDF1(int keyLength, String password, byte[] salt)
            ...
            while (insertPosition < keyLength) {
                ...
                System.arraycopy(round, 0, key, insertPosition,
                        Math.min(keyLength - insertPosition, round.length));
        ...
        private byte[] deriveKeyPBKDF2(String algorithm, String password, byte[] salt, int iterations,
