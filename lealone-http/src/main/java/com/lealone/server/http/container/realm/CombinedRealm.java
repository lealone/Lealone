...
public class CombinedRealm extends RealmBase {
    ...
    public Principal authenticate(String username, String clientDigest, String nonce, String nc,
            String cnonce, String qop, String realmName, String digestA2, String algorithm) {
        ...
        for (Realm realm : realms) {
            ...
            authenticatedUser = realm.authenticate(username, clientDigest, nonce, nc, cnonce, qop,
            ...
            if (authenticatedUser == null) {
                ...
                if (log.isTraceEnabled()) {
                    log.trace(sm.getString("combinedRealm.authSuccess", username,
                            realm.getClass().getName()));
                }
                break;
            }
        ...
        while (iter.hasNext()) {
            ...
            if (realm instanceof Lifecycle) {
                try {
                    ...
                    iter.remove();
                    log.error(sm.getString("combinedRealm.realmStartFail", realm.getClass().getName()),
                            e);
                }
            }
        }
        ...
        if (gssContext.isEstablished()) {
            ...
            for (Realm realm : realms) {
                ...
                if (authenticatedUser == null) {
                    ...
                    if (log.isTraceEnabled()) {
                        log.trace(sm.getString("combinedRealm.authSuccess", gssName,
                                realm.getClass().getName()));
                    }
                    break;
                }
    ...
    public Principal authenticate(GSSName gssName, GSSCredential gssCredential) {
        ...
        for (Realm realm : realms) {
            ...
            if (authenticatedUser == null) {
                if (log.isTraceEnabled()) {
                    log.trace(
                            sm.getString("combinedRealm.authFail", gssName, realm.getClass().getName()));
                }
            } else {
                if (log.isTraceEnabled()) {
                    log.trace(sm.getString("combinedRealm.authSuccess", gssName,
