...
import com.lealone.server.servlet.http.HttpServletResponseWrapper;
 ...
 *       <filter-class>com.lealone.server.http.container.filters.ExpiresFilter</filter-class>
 *       <init-param>
 ...
 * {@code com.lealone.server.http.container.filters.ExpiresFilter}.
 * </p>
 * <p>
 ...
 *
 * <code>
 * com.lealone.server.http.container.filters.ExpiresFilter.level = FINE
 * </code>
 * <p>
 * Sample of initialization log message :
 * </p>
 * <code>
 * Mar 26, 2010 2:01:41 PM com.lealone.server.http.container.filters.ExpiresFilter init
 ...
 * </p>
 * <code>
 * Mar 26, 2010 2:09:47 PM com.lealone.server.http.container.filters.ExpiresFilter onBeforeWriteResponseBody
 ...
 * </p>
 * <code>
 * Mar 26, 2010 2:10:27 PM com.lealone.server.http.container.filters.ExpiresFilter onBeforeWriteResponseBody
...
public class ExpiresFilter extends FilterBase {
    ...
    protected static int[] commaDelimitedListToIntArray(String commaDelimitedInts) {
        ...
        for (int i = 0; i < intsAsStrings.length; i++) {
            ...
            try {
                ...
                throw new RuntimeException(sm.getString("expiresFilter.numberError", Integer.valueOf(i),
                        commaDelimitedInts));
            }
        }
        return ints;
        ...
        if (request instanceof HttpServletRequest httpRequest
            ...
            if (response.isCommitted()) {
                if (log.isDebugEnabled()) {
                    log.debug(sm.getString("expiresFilter.responseAlreadyCommitted",
    ...
    protected Date getExpirationDate(ExpiresConfiguration configuration, XHttpServletResponse response) {
        ...
        case LAST_MODIFICATION_TIME:
            ...
            break;
        default:
            throw new IllegalStateException(sm.getString("expiresFilter.unsupportedStartingPoint",
        ...
        for (Enumeration<String> names = filterConfig.getInitParameterNames(); names
            ...
            try {
                ...
                throw new ServletException(
                        sm.getString("expiresFilter.exceptionProcessingParameter", name, value), e);
            }
        }
        ...
        if (response.containsHeader(HEADER_EXPIRES)
            ...
            if (log.isDebugEnabled()) {
                log.debug(sm.getString("expiresFilter.expirationHeaderAlreadyDefined",
                        request.getRequestURI(), Integer.valueOf(response.getStatus()),
     ...
     * </p>
     * <p>
     * Invocations to {@code Logger.debug(...)} are guarded by {@link Logger#isDebugEnabled()} because
    ...
    protected ExpiresConfiguration parseExpiresConfiguration(String inputLine) {
        ...
        if ("access".equalsIgnoreCase(currentToken) || "now".equalsIgnoreCase(currentToken)) {
            ...
            throw new IllegalStateException(
                    sm.getString("expiresFilter.startingPointInvalid", currentToken, line));
        }
        ...
        while (currentToken != null) {
            ...
            try {
                ...
                throw new IllegalStateException(sm.getString("expiresFilter.noDurationUnitAfterAmount",
            ...
            if ("year".equalsIgnoreCase(currentToken) || "years".equalsIgnoreCase(currentToken)) {
                ...
                throw new IllegalStateException(
                        sm.getString("expiresFilter.invalidDurationUnit", currentToken, line));
            }
