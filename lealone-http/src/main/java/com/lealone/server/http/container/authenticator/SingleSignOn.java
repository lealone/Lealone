 ...
 * <code>com.lealone.server.http.container.authenticator</code> package.</li>
 * </ul>
 * <p>
...
public class SingleSignOn extends ValveBase {
    ...
    public void invoke(Request request, Response response) throws IOException, ServletException {
        ...
        if (entry != null) {
            ...
            if (containerLog.isDebugEnabled()) {
                containerLog
                        .debug(sm.getString("singleSignOn.debug.principalNotFound", cookie.getValue()));
            }
     ...
     */
    protected void register(String ssoId, Principal principal, String authType, String username,
            String password) {
     ...
     */
    protected boolean update(String ssoId, Principal principal, String authType, String username,
            String password) {
        ...
        if (containerRealm == null) {
            ...
            if (container instanceof Engine) {
                    ...
                    if (host.getRealm() != containerRealm) {
                        ...
                        for (Container context : host.findChildren()) {
                            if (context.getRealm() != containerRealm) {
                                containerLog.warn(
                                        sm.getString("singleSignOn.duplicateRealm", context.getName()));
                            }
                        }
                    }
                }
                ...
                for (Container context : container.findChildren()) {
                    if (context.getRealm() != containerRealm) {
                        containerLog
                                .warn(sm.getString("singleSignOn.duplicateRealm", context.getName()));
                    }
                }
            }
        }
    ...
    protected void sessionChangedId(String ssoId, Session session, String oldSessionId) {
        if (containerLog.isDebugEnabled()) {
            containerLog.debug(
                    sm.getString("singleSignOn.debug.sessionChangedId", session, oldSessionId, ssoId));
        }
