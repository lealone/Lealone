...
public class WebdavIfHeader {
    ...
    private static final Logger log = LoggerFactory.getLogger(WebdavIfHeader.class);
    private static final StringManager sm = StringManager.getManager(WebdavIfHeader.class);
    ...
    private class IfHeaderMap extends HashMap<String, IfHeaderList> implements IfHeaderInterface {
        ...
        public boolean matches(String resource, List<String> tokens, String etag) {
            if (log.isTraceEnabled()) {
                log.trace("matches: Trying to match resource=" + resource + ", token=" + tokens + ","
                        + etag);
            }
