...
import com.lealone.server.http.container.AsyncDispatcher;
...
import com.lealone.server.servlet.ServletRequestWrapper;
 ...
 * <code>com.lealone.server.servlet.ServletRequestWrapper</code> and <code>com.lealone.server.servlet.ServletResponseWrapper</code>.
 */
final class ApplicationDispatcher implements AsyncDispatcher, RequestDispatcher {
    ...
    public void forward(ServletRequest request, ServletResponse response)
        ...
        if (!finished) {
            ...
            if (wrapper.getLogger().isDebugEnabled()) {
                wrapper.getLogger().debug(
                        sm.getString("applicationDispatcher.customResponse", response.getClass()));
            }
            // Close anyway
            try {
    ...
    private void invoke(ServletRequest request, ServletResponse response, State state)
        ...
        if (wrapper.isUnavailable()) {
            wrapper.getLogger()
                    .warn(sm.getString("applicationDispatcher.isUnavailable", wrapper.getName()));
        ...
        try {
            ...
            wrapper.getLogger().error(
                    sm.getString("applicationDispatcher.allocateException", wrapper.getName()),
                    StandardWrapper.getRootCause(e));
            servletException = e;
        } catch (Throwable t) {
            ExceptionUtils.handleThrowable(t);
            wrapper.getLogger().error(
                    sm.getString("applicationDispatcher.allocateException", wrapper.getName()), t);
        ...
        ApplicationFilterChain filterChain = ApplicationFilterFactory.createFilterChain(request, wrapper,
                servlet);
        ...
        try {
            ...
            wrapper.getLogger().error(
                    sm.getString("applicationDispatcher.serviceException", wrapper.getName()), ioe);
            ioException = ioe;
        } catch (UnavailableException e) {
            wrapper.getLogger()
                    .error(sm.getString("applicationDispatcher.serviceException", wrapper.getName()), e);
            servletException = e;
            ...
            if (!(rootCause instanceof BadRequestException)) {
                wrapper.getLogger().error(
                        sm.getString("applicationDispatcher.serviceException", wrapper.getName()),
                        rootCause);
            }
            servletException = e;
