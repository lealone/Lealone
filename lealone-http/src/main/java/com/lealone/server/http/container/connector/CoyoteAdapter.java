...
import com.lealone.server.http.container.authenticator.AuthenticatorBase;
import com.lealone.server.http.container.core.AsyncContextImpl;
import com.lealone.server.http.container.util.ServerInfo;
import com.lealone.server.http.container.util.SessionConfig;
...
import com.lealone.server.http.util.buf.CharChunk;
...
import com.lealone.server.servlet.ReadListener;
...
import com.lealone.server.servlet.WriteListener;
...
public class CoyoteAdapter implements Adapter {
    ...
    private static final String POWERED_BY = "Servlet/6.1 JSP/4.0 " + "(" + ServerInfo.getServerInfo()
            + " Java/" + System.getProperty("java.vm.vendor") + "/"
    ...
    @Override
    public boolean asyncDispatch(com.lealone.server.http.protocol.Request req,
            com.lealone.server.http.protocol.Response res, SocketEvent status) throws Exception {
    ...
    @Override
    public void service(com.lealone.server.http.protocol.Request req,
            com.lealone.server.http.protocol.Response res) throws Exception {
    ...
    @Override
    public boolean prepare(com.lealone.server.http.protocol.Request req,
            com.lealone.server.http.protocol.Response res) throws IOException, ServletException {
        Request request = (Request) req.getNote(ADAPTER_NOTES);
        Response response = (Response) res.getNote(ADAPTER_NOTES);
    ...
    @Override
    public void log(com.lealone.server.http.protocol.Request req,
            com.lealone.server.http.protocol.Response res, long time) {
    ...
    @Override
    public void checkRecycled(com.lealone.server.http.protocol.Request req,
            com.lealone.server.http.protocol.Response res) {
        Request request = (Request) req.getNote(ADAPTER_NOTES);
        Response response = (Response) res.getNote(ADAPTER_NOTES);
     ...
     */
    protected boolean postParseRequest(com.lealone.server.http.protocol.Request req, Request request,
            com.lealone.server.http.protocol.Response res, Response response)
        ...
        while (mapRequired) {
            ...
            connector.getService().getMapper().map(serverName, decodedURI, version,
            ...
            if (request.getServletContext().getEffectiveSessionTrackingModes()
                ...
                sessionID = request
                        .getPathParameter(SessionConfig.getSessionUriParamName(request.getContext()));
                if (sessionID != null) {
    ...
    private void doConnectorAuthenticationAuthorization(com.lealone.server.http.protocol.Request req,
            Request request) {
     ...
     */
    protected void parsePathParameters(com.lealone.server.http.protocol.Request req, Request request) {
        ...
        while (semicolon > -1) {
            ...
            if (log.isTraceEnabled()) {
                log.trace(sm.getString("coyoteAdapter.debug", "pathParamStart",
