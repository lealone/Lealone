...
import com.lealone.server.http.protocol.CloseNowException;
import com.lealone.server.http.protocol.InputBuffer;
import com.lealone.server.http.protocol.Request;
import com.lealone.server.http.protocol.Response;
import com.lealone.server.http.protocol.http11.HttpOutputBuffer;
import com.lealone.server.http.protocol.http11.OutputFilter;
...
class Stream extends AbstractNonZeroStream implements HeaderEmitter {
    ...
    Stream(Integer identifier, Http2UpgradeHandler handler, Request coyoteRequest) {
        ...
        if (coyoteRequest == null) {
            ...
            this.inputBuffer = new SavedRequestStreamInputBuffer(
                    (SavedRequestInputFilter) this.coyoteRequest.getInputBuffer());
    ...
    final void receiveReset(long errorCode) {
        if (log.isTraceEnabled()) {
            log.trace(sm.getString("stream.reset.receive", getConnectionId(), getIdAsString(),
    ...
    public final void emitHeader(String name, String value) throws HpackException {
        if (log.isTraceEnabled()) {
            log.trace(sm.getString("stream.header.debug", getConnectionId(), getIdAsString(), name,
                    value));
        }
        ...
        if ("te".equals(name)) {
            if (!"trailers".equals(value)) {
                headerException = new StreamException(
                        sm.getString("stream.header.te", getConnectionId(), getIdAsString(), value),
                        Http2Error.PROTOCOL_ERROR, getIdAsInt());
                // No need for further processing. The stream will be reset.
                return;
            }
        ...
        if (name.isEmpty()) {
            headerException = new StreamException(
                    sm.getString("stream.header.empty", getConnectionId(), getIdAsString()),
                    Http2Error.PROTOCOL_ERROR, getIdAsInt());
            // No need for further processing. The stream will be reset.
            return;
        }
        ...
        if (pseudoHeader && headerState != HEADER_STATE_PSEUDO) {
            headerException = new StreamException(sm.getString("stream.header.unexpectedPseudoHeader",
                    getConnectionId(), getIdAsString(), name), Http2Error.PROTOCOL_ERROR, getIdAsInt());
            // No need for further processing. The stream will be reset.
            return;
        }
        ...
        case ":method": {
            if (coyoteRequest.getMethod() == null) {
                ...
                headerException = new StreamException(sm.getString("stream.header.duplicate",
                        getConnectionId(), getIdAsString(), ":method"), Http2Error.PROTOCOL_ERROR,
                        getIdAsInt());
            }
            break;
        }
        case ":scheme": {
            if (coyoteRequest.scheme().isNull()) {
                ...
                headerException = new StreamException(sm.getString("stream.header.duplicate",
                        getConnectionId(), getIdAsString(), ":scheme"), Http2Error.PROTOCOL_ERROR,
                        getIdAsInt());
            }
            break;
        }
        case ":path": {
            if (!coyoteRequest.requestURI().isNull()) {
                headerException = new StreamException(sm.getString("stream.header.duplicate",
                        getConnectionId(), getIdAsString(), ":path"), Http2Error.PROTOCOL_ERROR,
                        getIdAsInt());
        ...
        case ":authority": {
            if (coyoteRequest.serverName().isNull()) {
                ...
                headerException = new StreamException(sm.getString("stream.header.duplicate",
                        getConnectionId(), getIdAsString(), ":authority"), Http2Error.PROTOCOL_ERROR,
                        getIdAsInt());
            }
            break;
        }
        ...
        case "host": {
            if (coyoteRequest.serverName().isNull()) {
                ...
                headerException = new StreamException(sm.getString("stream.header.duplicate",
                        getConnectionId(), getIdAsString(), "host"), Http2Error.PROTOCOL_ERROR,
                        getIdAsInt());
            }
            break;
        }
            ...
            try {
                ...
                if (log.isTraceEnabled()) {
                    log.trace(sm.getString("http2Parser.processFramePriorityUpdate.invalid",
                            getConnectionId(), getIdAsString()), iae);
                }
            }
            break;
        }
        default: {
            ...
            if (pseudoHeader) {
                headerException = new StreamException(sm.getString("stream.header.unknownPseudoHeader",
                        getConnectionId(), getIdAsString(), name), Http2Error.PROTOCOL_ERROR,
                        getIdAsInt());
    ...
    private void parseAuthority(String value) {
        ...
        if (!handler.getProtocol().getHttp11Protocol().checkSni(handler.getSniHostName(),
            ...
            headerException = new StreamException(sm.getString("stream.host.sni", getConnectionId(),
                    getIdAsString(), value, handler.getSniHostName()), Http2Error.PROTOCOL_ERROR,
                    getIdAsInt());
        }
    }
        ...
        try {
            ...
            if (i == -1
                    ...
                    || i > -1 && ((!value.substring(0, i).equals(coyoteRequest.serverName().getString())
                            || Integer.parseInt(value.substring(i + 1)) != coyoteRequest
                                    .getServerPort()))) {
                // Host value inconsistent
                headerException = new StreamException(
                        sm.getString("stream.host.inconsistent", getConnectionId(), getIdAsString(),
                                value, coyoteRequest.serverName().getString(),
    ...
    final boolean receivedEndOfHeaders() throws StreamException {
        ...
        if (missingHeader) {
            throw new StreamException(
                    sm.getString("stream.header.required", getConnectionId(), getIdAsString()),
                    Http2Error.PROTOCOL_ERROR, getIdAsInt());
        }
    ...
    final void writeHeaders() throws IOException {
        ...
        handler.writeHeaders(this, coyoteResponse.getMimeHeaders(), endOfStream,
    ...
    final void receivedEndOfStream() throws ConnectionException {
        if (isContentLengthInconsistent()) {
            throw new ConnectionException(sm.getString("stream.header.contentLength", getConnectionId(),
                    getIdAsString(), Long.valueOf(coyoteRequest.getContentLengthLong()),
    ...
    final void close(Http2Exception http2Exception) {
        if (http2Exception instanceof StreamException) {
            try {
                ...
                if (log.isTraceEnabled()) {
                    log.trace(sm.getString("stream.reset.send", getConnectionId(), getIdAsString(),
                            se.getError()));
                }
