...
import com.lealone.server.http.protocol.NonPipeliningProcessor;
import com.lealone.server.http.protocol.Request;
import com.lealone.server.http.protocol.RequestGroupInfo;
...
class StreamProcessor extends AbstractProcessor implements NonPipeliningProcessor {
                    ...
                    if (!stream.isInputFinished() && getErrorState().isIoAllowed()) {
                        ...
                        if (se == null) {
                            se = new StreamException(
                                    sm.getString("streamProcessor.error.stream",
                                            stream.getConnectionId(), stream.getIdAsString()),
    ...
    @Override
    public final SocketState service(SocketWrapper<?> socket) throws IOException {
        try {
            if (validateRequest()) {
