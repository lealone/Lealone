...
import com.lealone.server.http.util.net.IPv6Utils;
...
public abstract class AbstractAccessLogValve extends ValveBase implements AccessLog {
     ...
     */
    private static final DateFormatCache globalDateCache = new DateFormatCache(globalCacheSize,
    ...
    private static final ThreadLocal<DateFormatCache> localDateCache = ThreadLocal.withInitial(
            () -> new DateFormatCache(localCacheSize, Locale.getDefault(), globalDateCache));
    ...
    protected interface AccessLogElement {
         ...
         */
        @Deprecated
        default void addElement(CharArrayWriter buf, Date date, Request request, Response response,
                long time) {
    ...
    protected static class ConnectionStatusElement implements AccessLogElement {
        ...
        public void addElement(CharArrayWriter buf, Request request, Response response, long time) {
            if (response != null && request != null) {
                ...
                if (!statusFound) {
                    String connStatus = response
                            .getHeader(com.lealone.server.http.protocol.http11.Constants.CONNECTION);
                    if (com.lealone.server.http.protocol.http11.Constants.CLOSE
                            .equalsIgnoreCase(connStatus)) {
                        buf.append('-');
                    } else {
                        buf.append('+');
                    }
