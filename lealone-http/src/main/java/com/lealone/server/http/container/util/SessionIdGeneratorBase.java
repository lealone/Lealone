 ...
 */
package com.lealone.server.http.container.util;
...
public abstract class SessionIdGeneratorBase extends LifecycleBase implements SessionIdGenerator {
    ...
    private static final StringManager sm = StringManager.getManager(SessionIdGeneratorBase.class);
    ...
    static {
        ...
        if (algorithmNames.contains("SHA1PRNG")) {
            ...
            Logger log = LoggerFactory.getLogger(SessionIdGeneratorBase.class);
    ...
    private SecureRandom createSecureRandom() {
        ...
        if (result == null) {
            ...
            try {
                ...
                error = true;
                log.error(sm.getString("sessionIdGeneratorBase.randomAlgorithm", secureRandomAlgorithm),
                        e);
            } catch (NoSuchProviderException e) {
                error = true;
                log.error(sm.getString("sessionIdGeneratorBase.randomProvider", secureRandomProvider),
                        e);
            }
        }
        ...
        if ((t2 - t1) > 100) {
            log.warn(sm.getString("sessionIdGeneratorBase.createRandom", result.getAlgorithm(),
                    Long.valueOf(t2 - t1)));
        }
        return result;
    }
