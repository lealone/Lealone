...
/**
 * Wrapper around a <code>com.lealone.server.servlet.ServletRequest</code> that transforms an application request object (which
 * might be the original one passed to a servlet, or might be based on the 2.3
 * <code>com.lealone.server.servlet.ServletRequestWrapper</code> class) back into an internal
 * <code>com.lealone.server.http.container.Request</code>.
 * <p>
...
class ApplicationRequest extends ServletRequestWrapper {
    ...
    private static final Set<String> specialsSet = new HashSet<>(
            Arrays.asList(RequestDispatcher.INCLUDE_REQUEST_URI, RequestDispatcher.INCLUDE_CONTEXT_PATH,
    ...
    private static final int shortestSpecialNameLength = specialsSet.stream().mapToInt(String::length)
            .min().getAsInt();
