...
import com.lealone.server.http.container.filters.CorsFilter;
import com.lealone.server.http.container.filters.RemoteIpFilter;
import com.lealone.server.http.container.realm.GenericPrincipal;
import com.lealone.server.http.container.util.FilterUtil;
import com.lealone.server.http.container.util.SessionIdGeneratorBase;
import com.lealone.server.http.container.util.StandardSessionIdGenerator;
import com.lealone.server.http.container.valves.RemoteIpValve;
import com.lealone.server.http.container.valves.ValveBase;
...
public abstract class AuthenticatorBase extends ValveBase implements Authenticator {
     ...
     * {@link com.lealone.server.http.container.authenticator.jaspic.CallbackHandlerImpl} will be used.
     */
    protected String jaspicCallbackHandlerClass = "com.lealone.server.http.container.authenticator.jaspic.CallbackHandlerImpl";
    ...
    public void setSsoReauthenticationMode(String ssoReauthenticationMode) {
        this.ssoReauthenticationMode = SsoReauthenticationMode
                .valueOf(ssoReauthenticationMode.trim().toUpperCase(Locale.ENGLISH));
    }
    ...
    public void setAllowCorsPreflight(String allowCorsPreflight) {
        this.allowCorsPreflight = AllowCorsPreflight
                .valueOf(allowCorsPreflight.trim().toUpperCase(Locale.ENGLISH));
    }
        ...
        if (cache) {
            ...
            if (principal == null) {
                ...
                if (session != null) {
                    ...
                    if (principal != null) {
                        if (log.isTraceEnabled()) {
                            log.trace("We have cached auth type " + session.getAuthType()
                                ...
                                for (FilterDef filterDef : request.getContext().findFilterDefs()) {
                                    if (CorsFilter.class.getName().equals(filterDef.getFilterClass())) {
                                        for (FilterMap filterMap : context.findFilterMaps()) {
                                            if (filterMap.getFilterName()
                                                    .equals(filterDef.getFilterName())) {
                                                if ((filterMap.getDispatcherMapping()
                                                        & FilterMap.REQUEST) > 0) {
                                                    String requestPath = FilterUtil
                                                            .getRequestPath(request);
                                                    if (FilterUtil.matchFiltersURL(filterMap,
                                                            requestPath)) {
                                                        allowBypass = true;
                                                    }
                                                }
    ...
    protected void register(Request request, HttpServletResponse response, Principal principal,
            String authType, String username, String password, boolean alwaysUseSession, boolean cache) {
