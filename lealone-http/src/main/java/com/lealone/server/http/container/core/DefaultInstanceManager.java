...
import com.lealone.server.http.util.collections.ManagedConcurrentWeakHashMap;
...
public class DefaultInstanceManager implements InstanceManager {
    ...
    public DefaultInstanceManager(Context context, Map<String, Map<String, String>> injectionMap,
            com.lealone.server.http.container.Context catalinaContext,
            ClassLoader containerClassLoader) {
        classLoader = catalinaContext.getClassLoader();
                    ...
                    if (context != null) {
                        ...
                        if (injections != null && Introspection.isValidSetter(method)) {
                            ...
                            if (injections.containsKey(fieldName)) {
                                annotations.add(new AnnotationCacheEntry(method.getName(),
                                        method.getParameterTypes(), injections.get(fieldName),
        ...
        while (clazz != null) {
            ...
            for (AnnotationCacheEntry entry : annotations) {
                if (entry.type() == AnnotationCacheEntryType.SETTER) {
                    lookupMethodResource(context, instance, getMethod(clazz, entry), entry.name(),
                            clazz);
    ...
    protected Class<?> loadClass(String className, ClassLoader classLoader)
            throws ClassNotFoundException {
        if (className.startsWith("com.lealone.server.http.container")) {
    ...
    private void checkAccess(Class<?> clazz) {
        ...
        if (ContainerServlet.class.isAssignableFrom(clazz)) {
            throw new SecurityException(
                    sm.getString("defaultInstanceManager.restrictedContainerServlet", clazz));
        }
        while (clazz != null) {
            if (restrictedClasses.contains(clazz.getName())) {
                throw new SecurityException(
                        sm.getString("defaultInstanceManager.restrictedClass", clazz));
            }
            clazz = clazz.getSuperclass();
        }
    }
     ...
     */
    protected static void lookupMethodResource(Context context, Object instance, Method method,
        ...
        if ((normalizedName != null) && (!normalizedName.isEmpty())) {
            ...
            lookedupResource = context
                    .lookup(clazz.getName() + "/" + Introspection.getPropertyName(method));
        }
