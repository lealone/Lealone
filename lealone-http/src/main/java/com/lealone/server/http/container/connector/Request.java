 ...
 */
package com.lealone.server.http.container.connector;
...
import com.lealone.server.http.container.TomcatPrincipal;
import com.lealone.server.http.container.Wrapper;
import com.lealone.server.http.container.core.ApplicationFilterChain;
import com.lealone.server.http.container.core.ApplicationMapping;
import com.lealone.server.http.container.core.ApplicationPart;
import com.lealone.server.http.container.core.ApplicationSessionCookieConfig;
import com.lealone.server.http.container.core.AsyncContextImpl;
import com.lealone.server.http.container.mapper.MappingData;
import com.lealone.server.http.container.util.ParameterMap;
import com.lealone.server.http.container.util.RequestUtil;
import com.lealone.server.http.container.util.TLSUtil;
...
import com.lealone.server.http.protocol.UpgradeToken;
...
import com.lealone.server.http.protocol.util.InvalidParameterException;
import com.lealone.server.http.protocol.util.Parameters;
import com.lealone.server.http.protocol.util.Rfc6265CookieProcessor;
import com.lealone.server.http.protocol.util.ServerCookie;
import com.lealone.server.http.protocol.util.ServerCookies;
import com.lealone.server.http.protocol.util.fileupload.FileItem;
import com.lealone.server.http.protocol.util.fileupload.FileUpload;
import com.lealone.server.http.protocol.util.fileupload.disk.DiskFileItemFactory;
import com.lealone.server.http.protocol.util.fileupload.impl.FileCountLimitExceededException;
import com.lealone.server.http.protocol.util.fileupload.impl.InvalidContentTypeException;
import com.lealone.server.http.protocol.util.fileupload.impl.SizeException;
import com.lealone.server.http.protocol.util.fileupload.servlet.ServletRequestContext;
import com.lealone.server.http.protocol.util.parser.AcceptLanguage;
import com.lealone.server.http.protocol.util.parser.MediaType;
import com.lealone.server.http.protocol.util.parser.Upgrade;
import com.lealone.server.http.util.ExceptionUtils;
import com.lealone.server.http.util.buf.B2CConverter;
import com.lealone.server.http.util.buf.ByteChunk;
import com.lealone.server.http.util.buf.CharsetHolder;
...
import com.lealone.server.http.util.buf.UDecoder;
import com.lealone.server.http.util.net.SSLSupport;
import com.lealone.server.http.util.res.StringManager;
import com.lealone.server.servlet.AsyncContext;
import com.lealone.server.servlet.DispatcherType;
import com.lealone.server.servlet.FilterChain;
import com.lealone.server.servlet.MultipartConfigElement;
...
import com.lealone.server.servlet.ServletRequestAttributeEvent;
...
import com.lealone.server.servlet.http.Cookie;
import com.lealone.server.servlet.http.HttpServletMapping;
...
import com.lealone.server.servlet.http.HttpSession;
...
public class Request implements HttpServletRequest {
     ...
     *                          without triggering an NPE.
     */
    public Request(Connector connector, com.lealone.server.http.protocol.Request coyoteRequest) {
     ...
     * Coyote request.
     */
    protected final com.lealone.server.http.protocol.Request coyoteRequest;
     ...
     */
    public com.lealone.server.http.protocol.Request getCoyoteRequest() {
    ...
    private Charset getCharset() {
        ...
        if (charset == null) {
            charset = com.lealone.server.http.protocol.Constants.DEFAULT_BODY_CHARSET;
        }
    ...
    public RequestDispatcher getRequestDispatcher(String path) {
        ...
        if (context.getDispatchersUseEncodedPaths()) {
            if (pos >= 0) {
                relative = URLEncoder.DEFAULT.encode(requestPath.substring(0, pos + 1),
    ...
    public void removeAttribute(String name) {
        ...
        if (name.startsWith("com.lealone.http.")) {
    ...
    private void notifyAttributeAssigned(String name, Object value, Object oldValue) {
        ...
        if (replaced) {
            event = new ServletRequestAttributeEvent(context.getServletContext(), getRequest(), name,
                    oldValue);
        } else {
            event = new ServletRequestAttributeEvent(context.getServletContext(), getRequest(), name,
                    value);
    ...
    public AsyncContext startAsync(ServletRequest request, ServletResponse response) {
        if (!isAsyncSupported()) {
            IllegalStateException ise = new IllegalStateException(
                    sm.getString("request.asyncNotSupported"));
            log.warn(sm.getString("coyoteRequest.noAsync", StringUtils.join(getNonAsyncClassNames())),
                    ise);
            throw ise;
        }
    ...
    public String getContextPath() {
        ...
        candidate = com.lealone.server.http.protocol.util.RequestUtil.normalize(candidate);
    ...
    public void changeSessionId(String newSessionId) {
        ...
        if (response != null && context != null) {
            Cookie newCookie = ApplicationSessionCookieConfig.createSessionCookie(context, newSessionId,
                    isSecure());
        ...
        if (requestedSessionId != null) {
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.debug(
                            sm.getString("request.session.failed", requestedSessionId, ioe.getMessage()),
                            ioe);
                } else {
                    log.info(sm.getString("request.session.failed", requestedSessionId,
                            ioe.getMessage()));
                }
                session = null;
            }
    ...
    protected void doParseParameters() {
        ...
        if (len > 0) {
            ...
            if ((maxPostSize >= 0) && (len > maxPostSize)) {
                ...
                checkSwallowInput();
                parametersParseException = new InvalidParameterException(message,
                        HttpServletResponse.SC_REQUEST_ENTITY_TOO_LARGE);
                return;
            }
            byte[] formData;
