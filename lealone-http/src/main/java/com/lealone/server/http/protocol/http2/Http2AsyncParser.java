...
import com.lealone.server.http.util.net.SocketWrapper.CompletionCheck;
import com.lealone.server.http.util.net.SocketWrapper.CompletionHandlerCall;
...
class Http2AsyncParser extends Http2Parser {
    ...
    void readConnectionPreface(WebConnection webConnection, Stream stream) throws Http2Exception {
        ...
        socketWrapper.read(BlockingMode.NON_BLOCK, socketWrapper.getReadTimeout(), TimeUnit.MILLISECONDS,
                null, handler, handler, preface, header, framePayload);
    }
    ...
    private class PrefaceCompletionHandler extends FrameCompletionHandler {
        ...
        @Override
        public CompletionHandlerCall callHandler(CompletionState state, ByteBuffer[] buffers, int offset,
                int length) {
        ...
        public void completed(Long result, Void attachment) {
            if (streamException || error == null) {
                ...
                upgradeHandler.closeConnection(
                        new ConnectionException(error.getMessage(), Http2Error.PROTOCOL_ERROR, error));
            }
    ...
    private class FrameCompletionHandler implements CompletionCheck, CompletionHandler<Long, Void> {
        ...
        public void failed(Throwable e, Void attachment) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(sm.getString("http2Parser.error", connectionId, Integer.valueOf(streamId),
                        frameType), e);
            }
