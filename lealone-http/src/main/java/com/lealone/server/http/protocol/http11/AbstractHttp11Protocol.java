...
import com.lealone.server.http.protocol.CompressionConfig;
import com.lealone.server.http.protocol.ContinueResponseTiming;
import com.lealone.server.http.protocol.Processor;
...
import com.lealone.server.http.protocol.http11.upgrade.UpgradeGroupInfo;
import com.lealone.server.http.protocol.http11.upgrade.UpgradeProcessorExternal;
import com.lealone.server.http.protocol.http11.upgrade.UpgradeProcessorInternal;
...
public abstract class AbstractHttp11Protocol<S> extends AbstractProtocol<S> {
    ...
    private void configureUpgradeProtocol(UpgradeProtocol upgradeProtocol) {
        ...
        if (httpUpgradeName != null && !httpUpgradeName.isEmpty()) {
            ...
            getLog().info(sm.getString("abstractHttp11Protocol.httpUpgradeConfigured", getName(),
                    httpUpgradeName));
        }
        ...
        if (alpnName != null && !alpnName.isEmpty()) {
            ...
            if (getEndpoint().isSSLEnabled()) {
                ...
                getLog().info(
                        sm.getString("abstractHttp11Protocol.alpnConfigured", getName(), alpnName));
            } else {
    ...
    @Override
    protected Processor createUpgradeProcessor(SocketWrapper<?> socket, UpgradeToken upgradeToken) {
        ...
        if (httpUpgradeHandler instanceof InternalHttpUpgradeHandler) {
            return new UpgradeProcessorInternal(socket, upgradeToken,
                    getUpgradeGroupInfo(upgradeToken.protocol()));
        } else {
