...
public class MapperListener extends LifecycleMBeanBase implements ContainerListener, LifecycleListener {
     ...
     */
    private static final StringManager sm = StringManager.getManager(MapperListener.class);
    ...
    private void registerContext(Context context) {
        ...
        for (Container container : context.findChildren()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(sm.getString("mapperListener.registerWrapper", container.getName(),
                        contextPath, service));
            }
        }

        mapper.addContextVersion(host.getName(), host, contextPath, context.getWebappVersion(), context,
                welcomeFiles, resources, wrappers);
    ...
    private void registerWrapper(Wrapper wrapper) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(sm.getString("mapperListener.registerWrapper", wrapper.getName(), contextPath,
                    service));
        }
    }
