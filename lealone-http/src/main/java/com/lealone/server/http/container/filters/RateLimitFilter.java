...
import com.lealone.server.http.container.util.RateLimiter;
 ...
 * implementation, <code>com.lealone.server.http.container.util.FastRateLimiter</code>, is optimized for efficiency and low overhead
 ...
 * implementation, <code>com.lealone.server.http.container.util.ExactRateLimiter</code>, is intended to provide a less efficient but
 ...
 * <code>com.lealone.server.http.container.util.RateLimiter</code> interface.
 * </p>
 * <p>
 ...
 * <code>com.lealone.server.http.container.filters.RateLimitFilter.Count</code> and decide how to handle the request based on other
...
public class RateLimitFilter extends FilterBase {
     ...
     */
    public static final String RATE_LIMIT_ATTRIBUTE_COUNT = "com.lealone.server.http.container.filters.RateLimitFilter.Count";

    transient RateLimiter rateLimiter;

    private String rateLimitClassName = "com.lealone.server.http.container.util.FastRateLimiter";
    ...
    private final transient Logger log = LoggerFactory.getLogger(RateLimitFilter.class);
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (exposeHeaders) {
            ((HttpServletResponse) response).addHeader(HEADER_RATE_LIMIT_POLICY,
                    rateLimiter.getPolicy());
            if (enforce) {
