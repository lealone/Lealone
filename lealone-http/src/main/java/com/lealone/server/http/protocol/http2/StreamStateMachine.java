...
class StreamStateMachine {
    ...
    private void stateChange(State oldState, State newState) {
        if (state == oldState) {
            ...
            if (log.isTraceEnabled()) {
                log.trace(sm.getString("streamStateMachine.debug.change", connectionId, streamId,
                        oldState, newState));
            }
        }
    }
        ...
        if (!isFrameTypePermitted(frameType)) {
            if (state.connectionErrorForInvalidFrame) {
                throw new ConnectionException(sm.getString("streamStateMachine.invalidFrame",
                        connectionId, streamId, state, frameType), state.errorCodeForInvalidFrame);
            } else {
                throw new StreamException(sm.getString("streamStateMachine.invalidFrame", connectionId,
                        streamId, state, frameType), state.errorCodeForInvalidFrame,
    ...
    private enum State {
        ...
        State(boolean canRead, boolean canWrite, boolean canReset,
                boolean connectionErrorForInvalidFrame, Http2Error errorCode, FrameType... frameTypes) {
