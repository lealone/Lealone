...
import com.lealone.server.http.util.net.jsse.JSSEKeyManager;
import com.lealone.server.http.util.net.jsse.PEMFile;
...
public abstract class SSLUtilBase implements SSLUtil {
    ...
    protected SSLUtilBase(SSLHostConfigCertificate certificate, boolean warnTls13) {
        ...
        List<String> enabledProtocols = getEnabled("protocols", getLog(), warnTls13, configuredProtocols,
        ...
        if (!JreCompat.isJre22Available() && sslHostConfig.getCiphers().startsWith("PROFILE=")) {
            ...
            boolean warnOnSkip = !sslHostConfig.getCiphers()
                    .equals(SSLHostConfig.DEFAULT_TLS_CIPHERS_12);
            List<String> configuredCiphers = sslHostConfig.getJsseCipherNames();
            Set<String> implementedCiphers = getImplementedCiphers();
            List<String> enabledCiphers = getEnabled("ciphers", getLog(), warnOnSkip, configuredCiphers,
                    implementedCiphers);
    ...
    static KeyStore getStore(String type, String provider, String path, String pass, String passFile)
        ...
        try {
            ...
            if ("DKS".equalsIgnoreCase(type)) {
                ...
                // to be null
                if (!("PKCS11".equalsIgnoreCase(type) || path.isEmpty()
                ...
                if (passToUse != null && (!passToUse.isEmpty() || "JKS".equalsIgnoreCase(type)
    ...
    public KeyManager[] getKeyManagers() throws Exception {
        ...
        if (ks == null) {
            ...
            PEMFile privateKeyFile = new PEMFile(
                    certificate.getCertificateKeyFile() != null ? certificate.getCertificateKeyFile()
        ...
        if (className != null && !className.isEmpty()) {
            ...
            if (!(TrustManager.class.isAssignableFrom(clazz))) {
                throw new InstantiationException(
                        sm.getString("sslUtilBase.invalidTrustManagerClassName", className));
            }
    ...
    protected CertPathParameters getParameters(final String crlf, final KeyStore trustStore,
        ...
        if (sslHostConfig.getOcspEnabled()) {
            PKIXRevocationChecker revocationChecker = (PKIXRevocationChecker) CertPathValidator
