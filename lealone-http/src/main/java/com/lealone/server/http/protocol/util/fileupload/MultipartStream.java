...
import com.lealone.server.http.protocol.util.fileupload.impl.SizeLimitExceededException;
    ...
    public String readHeaders() throws FileUploadIOException, MalformedStreamException {
        ...
        while (i < HEADER_SEPARATOR.length) {
            ...
            if (getPartHeaderSizeMax() != -1 && size > getPartHeaderSizeMax()) {
                throw new FileUploadIOException(new SizeLimitExceededException(String.format(
                        "Header section has more than %s bytes (maybe it is not properly terminated)",
                        Integer.valueOf(getPartHeaderSizeMax())), size, getPartHeaderSizeMax()));
            }
