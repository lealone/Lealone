 ...
 * </ul>
 *
 * @see com.lealone.server.http.container.Valve
 * @see Filter
 */
public class FilterValve extends ValveBase implements FilterConfig {
    ...
    protected void startInternal() throws LifecycleException {
        ...
        if (parent instanceof Context) {
            ...
            // final ScheduledExecutorService executor = Container.getService(super.getContainer())
        ...
        try {
            filter = (Filter) Class.forName(getFilterClassName()).getConstructor(new Class[0])
                    .newInstance();

            filter.init(this);
        } catch (ServletException | InstantiationException | IllegalAccessException
                | IllegalArgumentException | InvocationTargetException | NoSuchMethodException
                | SecurityException | ClassNotFoundException se) {
            throw new LifecycleException(sm.getString("filterValve.initError", getFilterClassName()),
                    se);
        }
    }
        ...
        if (!filterCallInfo.stop) {
            if (request != filterCallInfo.request) {
                throw new IllegalStateException(
                        sm.getString("filterValve.illegalWrapping", getFilterClassName()));
            }
