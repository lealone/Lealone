    ...
    public HeaderParseStatus parseHeader() throws IOException {
        ...
        while (headerParsePos == HeaderParsePosition.HEADER_NAME) {
            ...
            if (chr == COLON) {
                ...
                headerData.headerValue = headers.addValue(source.getHeaderByteBuffer().array(),
        ...
        while (headerParsePos == HeaderParsePosition.HEADER_VALUE_START
            ...
            if (headerParsePos == HeaderParsePosition.HEADER_VALUE_START) {
                ...
                while (true) {
                    // Read new bytes if needed
                    if (source.getHeaderByteBuffer().position() >= source.getHeaderByteBuffer()
                            .limit()) {
                        if (!source.fillHeaderBuffer()) {
                            return HeaderParseStatus.NEED_MORE_DATA;
                        }
                    ...
                    if (chr != SP && chr != HT) {
                        ...
                        source.getHeaderByteBuffer()
                                .position(source.getHeaderByteBuffer().position() - 1);
                    ...
                    if (chr == CR && prevChr != CR) {
                        ...
                        if (!tolerantEol && prevChr != CR) {
                            throw new IllegalArgumentException(
                                    sm.getString("httpHeaderParser.invalidCrlfNoCR"));
                        }
                        eol = true;
                    } else if (prevChr == CR) {
