...
class ByteUtil {
    ...
    static int getTwoBytes(ByteBuffer input, int pos) {
        return ((input.get(pos) & 0xFF) << 8) + (input.get(pos + 1) & 0xFF);
    }
    ...
    static long getFourBytes(ByteBuffer input, int pos) {
        return ((long) (input.get(pos) & 0xFF) << 24) + ((input.get(pos + 1) & 0xFF) << 16)
                + ((input.get(pos + 2) & 0xFF) << 8) + (input.get(pos + 3) & 0xFF);
    }
