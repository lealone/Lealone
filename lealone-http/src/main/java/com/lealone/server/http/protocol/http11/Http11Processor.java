...
import com.lealone.server.http.protocol.AbstractProcessor;
import com.lealone.server.http.protocol.ActionCode;
import com.lealone.server.http.protocol.Adapter;
import com.lealone.server.http.protocol.ContinueResponseTiming;
import com.lealone.server.http.protocol.ErrorState;
import com.lealone.server.http.protocol.Request;
import com.lealone.server.http.protocol.RequestInfo;
import com.lealone.server.http.protocol.UpgradeProtocol;
import com.lealone.server.http.protocol.UpgradeToken;
import com.lealone.server.http.protocol.http11.filters.BufferedInputFilter;
import com.lealone.server.http.protocol.http11.filters.ChunkedInputFilter;
import com.lealone.server.http.protocol.http11.filters.ChunkedOutputFilter;
import com.lealone.server.http.protocol.http11.filters.GzipOutputFilter;
import com.lealone.server.http.protocol.http11.filters.IdentityInputFilter;
import com.lealone.server.http.protocol.http11.filters.IdentityOutputFilter;
import com.lealone.server.http.protocol.http11.filters.SavedRequestInputFilter;
import com.lealone.server.http.protocol.http11.filters.VoidInputFilter;
import com.lealone.server.http.protocol.http11.filters.VoidOutputFilter;
import com.lealone.server.http.protocol.http11.upgrade.InternalHttpUpgradeHandler;
import com.lealone.server.http.protocol.http11.upgrade.UpgradeApplicationBufferHandler;
...
import com.lealone.server.http.protocol.util.parser.TokenList;
...
public class Http11Processor extends AbstractProcessor {
    ...
    private SocketState service0(SocketWrapper<?> socketWrapper) throws IOException {
        RequestInfo rp = request.getRequestProcessor();
        rp.setStage(com.lealone.server.http.protocol.Constants.STAGE_PARSE);
        ...
        while (!getErrorState().isError() && keepAlive && !isAsync() && upgradeToken == null
                    ...
                    if (upgradeProtocol.accept(request)) {
                        ...
                        if (upgradeRequest != null) {
                            ...
                            InternalHttpUpgradeHandler upgradeHandler = upgradeProtocol
                                    .getInternalUpgradeHandler(socketWrapper, getAdapter(),
                                            upgradeRequest);
            ...
            if (getErrorState().isIoAllowed()) {
                ...
                rp.setStage(com.lealone.server.http.protocol.Constants.STAGE_PREPARE);
                try {
                    prepareRequest();
                } catch (Throwable t) {
                    ExceptionUtils.handleThrowable(t);
            ...
            if (getErrorState().isIoAllowed()) {
                try {
                    rp.setStage(com.lealone.server.http.protocol.Constants.STAGE_SERVICE);
            ...
            rp.setStage(com.lealone.server.http.protocol.Constants.STAGE_ENDINPUT);
            if (!isAsync()) {
            ...
            rp.setStage(com.lealone.server.http.protocol.Constants.STAGE_ENDOUTPUT);
            ...
            rp.setStage(com.lealone.server.http.protocol.Constants.STAGE_KEEPALIVE);

            sendfileState = processSendfile(socketWrapper);
        }

        rp.setStage(com.lealone.server.http.protocol.Constants.STAGE_ENDED);
            ...
            if (uriBC.startsWith("://", pos)) {
                ...
                if (http11) {
                    ...
                    if (hostValueMB != null) {
                        ...
                        // the Host header
                        if (!hostValueMB.getByteChunk().equalsIgnoreCase(uriB, uriBCStart + pos,
                                slashPos - pos)) {
    ...
    private void prepareSendfile(OutputFilter[] outputFilters) {
        String fileName = (String) request
                .getAttribute(com.lealone.server.http.protocol.Constants.SENDFILE_FILENAME_ATTR);
        if (fileName == null) {
            sendfileData = null;
        } else {
            ...
            outputBuffer.addActiveFilter(outputFilters[Constants.VOID_FILTER]);
            contentDelimitation = true;
            long pos = ((Long) request
                    .getAttribute(com.lealone.server.http.protocol.Constants.SENDFILE_FILE_START_ATTR))
                            .longValue();
            long end = ((Long) request
                    .getAttribute(com.lealone.server.http.protocol.Constants.SENDFILE_FILE_END_ATTR))
                            .longValue();
    ...
    protected final void sslReHandShake() throws IOException {
        if (sslSupport != null) {
            ...
            ((BufferedInputFilter) inputFilters[Constants.BUFFERED_FILTER])
                    .setLimit(protocol.getMaxSavePostSize());
     ...
     */
    private SendfileState processSendfile(SocketWrapper<?> socketWrapper) {
