...
public class AccessLogValve extends AbstractAccessLogValve {
    ...
    public synchronized void backgroundProcess() {
        ...
        if (rotatable && checkForOldLogs && maxDays > 0) {
            long deleteIfLastModifiedBefore = System.currentTimeMillis()
                    - (maxDays * 24L * 60 * 60 * 1000);
                    ...
                    for (String oldAccessLog : oldAccessLogs) {
                        ...
                        if (match) {
                            ...
                            if (file.isFile() && file.lastModified() < deleteIfLastModifiedBefore) {
                                if (!file.delete()) {
                                    log.warn(sm.getString("accessLogValve.deleteFail",
    ...
    private File getDirectoryFile() {
        ...
        if (!dir.isAbsolute()) {
            dir = new File(getContainer().getBaseDir(), directory);
        }
        return dir;
    }
    ...
    protected synchronized void open() {
        ...
        try {
            writer = new PrintWriter(new BufferedWriter(
                    new OutputStreamWriter(new FileOutputStream(pathname, true), charset), 128000),
                    false);
            ...
            writer = null;
            currentLogFile = null;
            log.error(sm.getString("accessLogValve.openFail", pathname, System.getProperty("user.name")),
                    ioe);
        }
