...
import com.lealone.server.http.container.util.TomcatCSS;
...
public class ErrorReportValve extends ValveBase {
    ...
    protected void report(Request request, Response response, Throwable throwable) {
        ...
        sb.append("<title>");
        sb.append(
                smClient.getString("errorReportValve.statusHeader", String.valueOf(statusCode), reason));
        sb.append("</title>");
    ...
    /**
     * Print out a partial servlet stack trace (truncating at the last occurrence of com.lealone.server.servlet.).
     *
    ...
    protected String getPartialServletStackTrace(Throwable t) {
        ...
        for (int i = elements.length - 1; i >= 0; i--) {
            if ((elements[i].getClassName()
                    .startsWith("com.lealone.server.http.container.core.ApplicationFilterChain"))
        ...
        for (int i = 0; i < pos; i++) {
            if (!(elements[i].getClassName().startsWith("com.lealone.server.http.container.core."))) {
    ...
    private boolean sendErrorPage(String location, Response response) {
        ...
        if (!file.isAbsolute()) {
            file = new File(getContainer().getBaseDir(), location);
        }
        if (!file.isFile() || !file.canRead()) {
            getContainer().getLogger()
                    .warn(sm.getString("errorReportValve.errorPageNotFound", location));
            return false;
        }
        ...
        try (OutputStream os = response.getOutputStream(); InputStream is = new FileInputStream(file)) {
            ...
            getContainer().getLogger()
                    .warn(sm.getString("errorReportValve.errorPageIOException", location), ioe);
            return false;
        }
