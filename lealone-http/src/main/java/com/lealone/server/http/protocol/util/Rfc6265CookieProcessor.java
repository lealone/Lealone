 ...
 */
package com.lealone.server.http.protocol.util;
...
import com.lealone.server.http.protocol.util.parser.Cookie;
...
import com.lealone.server.http.util.descriptor.Constants;
...
public class Rfc6265CookieProcessor extends CookieProcessorBase {
    ...
    private static final StringManager sm = StringManager.getManager(Rfc6265CookieProcessor.class);
    ...
    @Override
    public String generateHeader(com.lealone.server.servlet.http.Cookie cookie,
    ...
    private void validateDomain(String domain) {
        ...
        while (i < chars.length) {
            ...
            if (!domainValid.get(cur)) {
                throw new IllegalArgumentException(
                        sm.getString("rfc6265CookieProcessor.invalidDomain", domain));
            }
    ...
    private void validatePath(String path) {
        ...
        for (char ch : chars) {
            if (ch < 0x20 || ch > 0x7E || ch == ';') {
                throw new IllegalArgumentException(
                        sm.getString("rfc6265CookieProcessor.invalidPath", path));
            }
        }
    }
    ...
    private void validateAttribute(String name, String value) {
        if (!HttpParser.isToken(name)) {
            throw new IllegalArgumentException(
                    sm.getString("rfc6265CookieProcessor.invalidAttributeName", name));
        }
