 ...
 * {@code
 * <Context>
 *     <Valve className="com.lealone.server.http.container.valves.ParameterLimitValve"
 * </Context>
 * }
...
public class ParameterLimitValve extends ValveBase {
    ...
    protected void initInternal() throws LifecycleException {
        super.initInternal();
        containerLog = LoggerFactory.getLogger(getContainer().getLogName() + ".parameterLimit");
    }
    ...
    protected void startInternal() throws LifecycleException {
        ...
        if (getContainer() instanceof Context) {
            ...
            if (containerLog.isDebugEnabled()) {
                if (is == null) {
                    containerLog.debug(
                            sm.getString("parameterLimitValve.noConfiguration", webInfResourcePath));
                } else {
                    containerLog.debug(
                            sm.getString("parameterLimitValve.readConfiguration", webInfResourcePath));
                }
            }
        } else {
            String resourceName = Container.getConfigPath(getContainer(), resourcePath);
            try {
                ConfigurationSource.Resource resource = ConfigFileLoader.getSource()
                        .getResource(resourceName);
                is = resource.getInputStream();
            } catch (IOException ioe) {
                if (containerLog.isDebugEnabled()) {
                    containerLog.debug(sm.getString("parameterLimitValve.noConfiguration", resourceName),
                            ioe);
                }
            }
        }
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (lastEqualsIndex == -1) {
                    throw new IllegalArgumentException(
                            sm.getString("parameterLimitValve.invalidLine", line));
                }
                ...
                Pattern pattern = Pattern
                        .compile(UDecoder.URLDecode(patternString, StandardCharsets.UTF_8));
                String[] limits = limitsString.split(",");
                if (limits.length == 1) {
                    urlPatternLimits.put(pattern,
                            new Integer[] { Integer.valueOf(limits[0]), null, null });
