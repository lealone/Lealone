...
public class HpackDecoder {
        ...
        if (index <= Hpack.STATIC_TABLE_LENGTH) {
            ...
            if (res == null) {
                throw new HpackException(
                        sm.getString("hpackdecoder.nullHeader", Integer.valueOf(index)));
            }
            return res.name;
        }
    }
    ...
    private void addEntryToHeaderTable(Hpack.HeaderField entry) {
        ...
        if (log.isTraceEnabled()) {
            log.trace(sm.getString("hpackdecoder.addDynamic", Integer.valueOf(index), entry.name,
                    entry.value));
        }
