...
public class RestCsrfPreventionFilter extends CsrfPreventionFilterBase {
        ...
        public boolean apply(HttpServletRequest request, HttpServletResponse response)
            ...
            if (getLogger().isDebugEnabled()) {
                getLogger().debug(sm.getString("restCsrfPreventionFilter.invalidNonce.debug",
                        ...
                        Boolean.valueOf(request.getRequestedSessionId() != null), session,
            ...
            if (Objects.nonNull(params) && params.length > 0) {
                ...
                for (String param : params) {
                    if (!Objects.equals(param, nonce)) {
                        if (getLogger().isDebugEnabled()) {
                            getLogger()
                                    .debug(sm.getString("restCsrfPreventionFilter.multipleNonce.debug",
                                            request.getMethod(), request.getRequestURI()));
                        }
                        return null;
                    }
    ...
    public void setPathsAcceptingParams(String pathsList) {
        if (Objects.nonNull(pathsList)) {
            Arrays.asList(pathsList.split(pathsDelimiter))
                    .forEach(e -> pathsAcceptingParams.add(e.trim()));
        }
    }
