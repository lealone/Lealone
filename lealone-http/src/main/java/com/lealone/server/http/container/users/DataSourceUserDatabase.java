 ...
 */
package com.lealone.server.http.container.users;
...
import com.lealone.server.http.container.Group;
import com.lealone.server.http.container.Role;
import com.lealone.server.http.container.User;
...
public class DataSourceUserDatabase extends SparseUserDatabase {
    ...
    private static final Logger log = LoggerFactory.getLogger(DataSourceUserDatabase.class);
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    if (rs.getString(1) != null) {
                        String description = (roleAndGroupDescriptionCol != null) ? rs.getString(2)
                                : null;
                        ArrayList<Role> groupRoles = new ArrayList<>();
                        if (groupName != null) {
                            groupName = groupName.trim();
                            try (PreparedStatement stmt2 = dbConnection
                                    .prepareStatement(preparedGroupRoles)) {
                ...
                for (Role role : removedRoles.values()) {
                    if (tempRelationDelete != null) {
                        try (PreparedStatement stmt = dbConnection
                                .prepareStatement(tempRelationDelete.toString())) {
                    ...
                    try (PreparedStatement stmt = dbConnection
                            .prepareStatement(tempRelationDelete2.toString())) {
                        stmt.setString(1, role.getRolename());
                        stmt.executeUpdate();
                    } catch (SQLException e) {
                        log.error(sm.getString("dataSourceUserDatabase.exception"), e);
        ...
        if (isGroupStoreDefined()) {
            ...
            if (!removedGroups.isEmpty()) {
                ...
                for (Group group : removedGroups.values()) {
                    try (PreparedStatement stmt = dbConnection
                            .prepareStatement(groupRoleRelationDelete)) {
        ...
        if (!removedUsers.isEmpty()) {
            ...
            for (User user : removedUsers.values()) {
                if (userRoleRelationDelete != null) {
                    try (PreparedStatement stmt = dbConnection
                            .prepareStatement(userRoleRelationDelete)) {
                ...
                if (userGroupRelationDelete != null) {
                    try (PreparedStatement stmt = dbConnection
                            .prepareStatement(userGroupRelationDelete)) {
                        stmt.setString(1, user.getUsername());
                        stmt.executeUpdate();
                    } catch (SQLException e) {
                        log.error(sm.getString("dataSourceUserDatabase.exception"), e);
