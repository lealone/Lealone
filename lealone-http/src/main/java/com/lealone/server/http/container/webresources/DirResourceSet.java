 ...
 */
package com.lealone.server.http.container.webresources;
...
import com.lealone.server.http.container.WebResourceLockSet;
import com.lealone.server.http.container.WebResourceRoot;
import com.lealone.server.http.container.WebResourceRoot.ResourceSetType;
import com.lealone.server.http.container.util.ResourceSet;
import com.lealone.server.http.protocol.util.RequestUtil;
import com.lealone.server.http.util.compat.JreCompat;
import com.lealone.server.http.util.concurrent.KeyedReentrantReadWriteLock;

/**
 * Represents a {@link com.lealone.server.http.container.WebResourceSet} based on a directory.
 */
public class DirResourceSet extends AbstractFileResourceSet implements WebResourceLockSet {
    ...
    /**
     * Creates a new {@link com.lealone.server.http.container.WebResourceSet} based on a directory.
     *
     * @param root         The {@link WebResourceRoot} this new {@link com.lealone.server.http.container.WebResourceSet} will be added
     *                         to.
     * @param webAppMount  The path within the web application at which this {@link com.lealone.server.http.container.WebResourceSet}
     ...
     *                         served.
     * @param internalPath The path within this new {@link com.lealone.server.http.container.WebResourceSet} where resources will be
     *                         served from.
     */
    ...
    public WebResource getResource(String path) {
        ...
        if (isPathMounted(path, webAppMount)) {
            ...
            try {
                ...
                return new FileResource(root, path, f, readOnly, getManifest(), this,
                        readOnly ? null : path);
            } finally {
                if (readLock != null) {
                    readLock.unlock();
    ...
    protected void checkType(File file) {
        if (!file.isDirectory()) {
            throw new IllegalArgumentException(sm.getString("dirResourceSet.notDirectory", getBase(),
    ...
    protected void initInternal() throws LifecycleException {
        ...
        if (JreCompat.getInstance().disableCanonCaches()) {
             ...
             */
            throw new IllegalStateException(sm.getString("dirResourceSet.canonCaches.enabled",
                    getFileBase(), getRoot().getContext().getName()));
        }
    }
