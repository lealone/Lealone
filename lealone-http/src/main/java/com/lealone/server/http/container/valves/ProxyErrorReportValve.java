...
public class ProxyErrorReportValve extends ErrorReportValve {
    private static final Logger log = LoggerFactory.getLogger(ProxyErrorReportValve.class);
    ...
    private String getRedirectUrl(Response response) {
        StringManager sm = StringManager.getManager(this.getClass().getSimpleName(),
                response.getLocale());
        String redirectUrl = null;
        try {
            redirectUrl = sm.getString(Integer.toString(response.getStatus()));
        } catch (MissingResourceException e) {
            // Ignore
        }
        if (redirectUrl == null) {
            try {
                redirectUrl = sm.getString(Integer.toString(0));
