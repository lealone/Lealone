...
import com.lealone.server.http.util.net.SocketWrapper.CompletionState;
...
public class Http2AsyncUpgradeHandler extends Http2UpgradeHandler {
    ...
    protected void writeSettings() {
        socketWrapper.write(BlockingMode.SEMI_BLOCK, protocol.getWriteTimeout(), TimeUnit.MILLISECONDS,
                null, SocketWrapper.COMPLETE_WRITE, errorCompletion,
    ...
    void writeBody(Stream stream, ByteBuffer data, int len, boolean finished) throws IOException {
        ...
        if (writable) {
            ...
            socketWrapper.write(BlockingMode.BLOCK, protocol.getWriteTimeout(), TimeUnit.MILLISECONDS,
                    null, SocketWrapper.COMPLETE_WRITE, applicationErrorCompletion,
        ...
        if (sendfile != null) {
            try {
                try (FileChannel channel = FileChannel.open(sendfile.path, StandardOpenOption.READ)) {
                    sendfile.mappedBuffer = channel.map(MapMode.READ_ONLY, sendfile.pos,
                            sendfile.end - sendfile.pos);
                }
                ...
                sendfile.connectionReservation = reserveWindowSize(sendfile.stream,
            ...
            if (writable) {
                ...
                socketWrapper.write(BlockingMode.SEMI_BLOCK, protocol.getWriteTimeout(),
                        TimeUnit.MILLISECONDS, sendfile, SocketWrapper.COMPLETE_WRITE_WITH_COMPLETION,
        ...
        public void completed(Long nBytes, SendfileData sendfile) {
            ...
            do {
                ...
                try {
                    if (sendfile.connectionReservation == 0) {
                        if (sendfile.streamReservation == 0) {
                            int reservation = (sendfile.end - sendfile.pos > Integer.MAX_VALUE)
                                    ? Integer.MAX_VALUE
                                    : (int) (sendfile.end - sendfile.pos);
                ...
                if (writable) {
                    ...
                    // surrounding loop.
                    completionState = socketWrapper.write(BlockingMode.SEMI_BLOCK,
                            protocol.getWriteTimeout(), TimeUnit.MILLISECONDS, sendfile,
                            SocketWrapper.COMPLETE_WRITE, this, ByteBuffer.wrap(header),
