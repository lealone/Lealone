 ...
 * <td>Value returned by {@link com.lealone.server.servlet.ServletRequest#getServerPort()} when the <code>protocolHeader</code>
 ...
 * <code>
 * &lt;Valve
 *   className="com.lealone.server.http.container.valves.RemoteIpValve"
        ...
        if (isInternal || isTrustedProxy(originalRemoteAddr)) {
            ...
            if (hostHeader != null) {
                ...
                if (hostHeaderValue != null) {
                    try {
                        ...
                        if (portIndex > -1) {
                            log.debug(sm.getString("remoteIpValve.invalidHostWithPort", hostHeaderValue,
                                    hostHeader));
            ...
            if (log.isTraceEnabled()) {
                log.trace("Incoming request " + request.getRequestURI() + " with originalRemoteAddr ["
                        + originalRemoteAddr + "], originalRemoteHost=[" + originalRemoteHost
                        + "], originalSecure=[" + originalSecure + "], originalScheme=[" + originalScheme
                        + "], originalServerName=[" + originalServerName + "], originalServerPort=["
                        + originalServerPort + "] will be seen as newRemoteAddr=["
                        + request.getRemoteAddr() + "], newRemoteHost=[" + request.getRemoteHost()
                        + "], newSecure=[" + request.isSecure() + "], newScheme=[" + request.getScheme()
                        + "], newServerName=[" + request.getServerName() + "], newServerPort=["
                        + request.getServerPort() + "]");
            }
        } else {
            if (log.isTraceEnabled()) {
                log.trace("Skip RemoteIpValve for request " + request.getRequestURI()
