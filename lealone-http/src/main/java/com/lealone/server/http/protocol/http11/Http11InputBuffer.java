...
public class Http11InputBuffer implements InputBuffer, ApplicationBufferHandler, HeaderDataSource {
    ...
    boolean parseRequestLine(boolean keptAlive, int connectionTimeout, int keepAliveTimeout)
        ...
        if (parsingRequestLinePhase == 2) {
            ...
            while (!space) {
                ...
                if (chr == Constants.SP || chr == Constants.HT) {
                    space = true;
                    request.setMethod(byteBuffer.array(), parsingRequestLineStart,
                    ...
                    throw new IllegalArgumentException(
                            sm.getString("iib.invalidmethod", invalidMethodValue));
                }
            }
        ...
        if (parsingRequestLinePhase == 4) {
            ...
            while (!space) {
                ...
                if (prevChr == Constants.CR && chr != Constants.LF) {
                    ...
                    throw new IllegalArgumentException(
                            sm.getString("iib.invalidRequestTarget", invalidRequestTarget));
                }
                if (chr == Constants.SP || chr == Constants.HT) {
                    space = true;
                    end = pos;
        ...
        if (parsingRequestLinePhase == 6) {
            ...
            while (!parsingRequestLineEol) {
                ...
                if (chr == Constants.CR) {
                    ...
                    throw new IllegalArgumentException(
                            sm.getString("iib.invalidHttpProtocol", invalidProtocol));
                }
            }
        ...
        throw new IllegalStateException(
                sm.getString("iib.invalidPhase", Integer.valueOf(parsingRequestLinePhase)));
    }
    ...
    private boolean fill(boolean block) throws IOException {
        ...
        if (log.isTraceEnabled()) {
            log.trace("Before fill(): parsingHeader: [" + parsingHeader + "], parsingRequestLine: ["
                    ...
                    + "], byteBuffer.position(): [" + byteBuffer.position() + "], byteBuffer.limit(): ["
        ...
        try {
            ...
            SocketWrapper<?> socketWrapper = this.wrapper;
        ...
        if (log.isTraceEnabled()) {
            log.trace("Received [" + new String(byteBuffer.array(), byteBuffer.position(),
