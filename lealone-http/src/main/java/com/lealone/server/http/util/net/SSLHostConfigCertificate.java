...
import com.lealone.server.http.util.net.openssl.ciphers.Authentication;
...
public class SSLHostConfigCertificate implements Serializable {
    ...
    private static final String DEFAULT_KEYSTORE_FILE = System.getProperty("user.home") + File.separator
            + ".keystore";
    ...
    public void setCertificateKeystorePasswordFile(String certificateKeystorePasswordFile) {
        sslHostConfig.setProperty("Certificate.certificateKeystorePasswordFile",
    ...
    private void setStoreType(String name, StoreType type) {
        if (storeType == null) {
            ...
            log.warn(sm.getString("sslHostConfigCertificate.mismatch", name, sslHostConfig.getHostName(),
                    type, this.storeType));
        }
    }
