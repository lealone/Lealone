...
public class AOStoreTest extends TestBase {
    ...
    void testUnusedChunks() {
        ArrayList<BTreeChunk> unusedChunks = map.getStore().findUnusedChunks();
        for (BTreeChunk c : unusedChunks)
            p("id=" + c.id + ", pageCountLive=" + c.pageCountLive);

        map.getStore().freeUnusedChunks();
    }
    ...
    void openMap() {
        map = store.openBTreeMap("test", StringDataType.INSTANCE, StringDataType.INSTANCE);
        p(store.getMapNames());
    }

    void openRTreeMap() {
        rmap = store.openRTreeMap("rtest", StringDataType.INSTANCE, 3);
        p(store.getMapNames());
    }
