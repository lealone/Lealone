...
public class BTreeStore {
    ...
    private final String btreeStoreName;
     ...
     * Even if disabled, the store may contain (old) compressed pages.
     */
     ...
     */
    protected BTreeStore(BTreeMap<Object, Object> map) {
        this.map = map;
        btreeStoreName = map.getBTreeStoreName();
        ...
        if (!FileUtils.exists(btreeStoreName))
            FileUtils.createDirectories(btreeStoreName);
        String[] files = new File(btreeStoreName).list();
        if (files != null && files.length > 0) {
            for (String f : files) {
                int id = Integer.parseInt(f.substring(0, f.length() - AOStore.SUFFIX_AO_FILE_LENGTH));
                if (id > lastChunkId)
                    lastChunkId = id;
    ...
    private FileStore getFileStore(int chunkId) {
        String chunkFileName = btreeStoreName + File.separator + chunkId + AOStore.SUFFIX_AO_FILE;
        FileStore fileStore = new FileStore();
        fileStore.open(chunkFileName, map.config);
        return fileStore;
    }

    private synchronized BTreeChunk readChunkHeader(int chunkId) {
        FileStore fileStore = getFileStore(chunkId);

        BTreeChunk chunk = null;
        ByteBuffer chunkHeaderBlocks = fileStore.readFully(0, CHUNK_HEADER_SIZE);
    ...
    private void writeChunkHeader(BTreeChunk chunk) {
        ...
        header.put(bytes);
        header.rewind();
        write(chunk.fileStore, 0, header);
    }

    private void write(FileStore fileStore, long pos, ByteBuffer buffer) {
        try {
    ...
    private void readAllChunks(boolean readPagePositions) {
        ...
        if (files != null && files.length > 0) {
            for (String f : files) {
                int chunkId = Integer.parseInt(f.substring(0, f.length() - AOStore.SUFFIX_AO_FILE_LENGTH));
            ...
            if (readPagePositions) {
                for (BTreeChunk c : chunks.values()) {
                    if (c.pagePositions == null) {
                        ByteBuffer pagePositions = c.fileStore.readFully(c.pagePositionsOffset + CHUNK_HEADER_SIZE,
                                c.pageCount * 2 * 8);
    ...
    private long save() {
        ...
        c.fileStore = getFileStore(c.id);

        // chunk header
        writeChunkHeader(c);
        // chunk body
        write(c.fileStore, CHUNK_HEADER_SIZE, buff.getBuffer());
        c.fileStore.sync();

        for (BTreeChunk chunk : chunks.values()) {
            if (chunk.changed) {
                writeChunkHeader(chunk);
                chunk.fileStore.sync();
                chunk.changed = false;
            }
        }
    ...
    public void sync() { // TODO 不是必须的
        checkOpen();
        if (lastChunk != null && lastChunk.fileStore != null)
            lastChunk.fileStore.sync();
    }
    ...
    BTreePage readPage(long pos) {
        ...
        if (p == null) {
            ...
            p = BTreePage.read(c.fileStore, pos, map, filePos, maxPos);
    ...
    public synchronized void renameMap(BTreeMap<?, ?> map, String newName) { // TODO
        ...
        String fileName = (String) map.config.get("storeName");
        if (fileName != null) {
            ...
            FileUtils.move(btreeStoreName, fileName);
            // btreeStoreName = fileName;
        }
    }

    /**
     * Remove this store. Please note rolling back this operation does not restore
    ...
    public void writeInBackground(int autoCommitDelay) {
        ...
        if (autoCompactFillRate > 0 && lastChunk != null && lastChunk.fileStore != null) {
            FileStore fileStore = lastChunk.fileStore;
            try {
