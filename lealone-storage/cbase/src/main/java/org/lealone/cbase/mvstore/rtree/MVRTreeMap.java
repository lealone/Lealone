 ...
 */
package org.lealone.cbase.mvstore.rtree;

import java.util.ArrayList;
import java.util.Iterator;

import org.lealone.cbase.mvstore.CursorPos;
import org.lealone.cbase.mvstore.DataUtils;
import org.lealone.cbase.mvstore.MVMap;
import org.lealone.cbase.mvstore.Page;
...
public class MVRTreeMap<V> extends MVMap<SpatialKey, V> {
        ...
        if (alwaysAdd || get(key) == null) {
            if (p.getMemory() > store.getPageSplitSize() && p.getKeyCount() > 3) {
                ...
                Page.PageReference[] children = { new Page.PageReference(p, p.getPos(), p.getTotalCount()),
                        new Page.PageReference(split, split.getPos(), split.getTotalCount()), new Page.PageReference(null, 0, 0) };
    ...
    private Page newPage(boolean leaf, long writeVersion) {
        ...
        if (leaf) {
            ...
            values = null;
            refs = new Page.PageReference[] { new Page.PageReference(null, 0, 0) };
        }
        return Page.create(this, writeVersion, Page.EMPTY_OBJECT_ARRAY, values, refs, 0, 0);
    }
