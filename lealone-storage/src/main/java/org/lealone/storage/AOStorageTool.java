...
//
// import org.lealone.aostore.btree.BTreeChunk;
...
//
// /**
// * Utility methods used in combination with the AOStore.
// */
// public class AOStoreTool {
//
// /**
...
// * <tr>
// * <td>[-dump &lt;fileName&gt;]</td>
// * <td>Dump the contends of the file</td>
// * </tr>
// * <tr>
// * <td>[-info &lt;fileName&gt;]</td>
// * <td>Get summary information about a file</td>
// * </tr>
// * <tr>
// * <td>[-compact &lt;fileName&gt;]</td>
// * <td>Compact a store</td>
// * </tr>
// * <tr>
// * <td>[-compress &lt;fileName&gt;]</td>
// * <td>Compact a store with compression enabled</td>
// * </tr>
// * </table>
// *
...
// int blockSize = BTreeStore.BLOCK_SIZE;
...
// BTreeChunk c = null;
// try {
// c = BTreeChunk.readChunkHeader(block, pos);
...
// continue;
// }
// int length = c.len * BTreeStore.BLOCK_SIZE;
...
// }
// int footerPos = chunk.limit() - BTreeChunk.FOOTER_LENGTH;
// try {
// chunk.position(footerPos);
// pw.printf("+%0" + len + "x chunkFooter %s%n", footerPos, new String(chunk.array(),
// chunk.position(), BTreeChunk.FOOTER_LENGTH, DataUtils.LATIN).trim());
// } catch (IllegalArgumentException e) {
// // too far
...
// AOStore store = new AOStore.Builder().storeName(fileName).readOnly().open();
// BTreeStore mapStore = new BTreeStore(fileName, store.getConfig());
// try {
// BTreeMap<String, String> meta = mapStore.getMetaMap();
// Map<String, Object> header = mapStore.getStoreHeader();
// long fileCreated = DataUtils.readHexLong(header, "created", 0L);
// TreeMap<Integer, BTreeChunk> chunks = new TreeMap<Integer, BTreeChunk>();
// long chunkLength = 0;
// long maxLength = 0;
...
// BTreeChunk c = BTreeChunk.fromString(e.getValue());
// chunks.put(c.id, c);
// chunkLength += c.len * BTreeStore.BLOCK_SIZE;
// maxLength += c.maxLen;
...
// for (Entry<Integer, BTreeChunk> e : chunks.entrySet()) {
// BTreeChunk c = e.getValue();
...
// String tempName = fileName + AOStore.SUFFIX_AO_STORE_TEMP_FILE;
// FileUtils.delete(tempName);
// compact(fileName, tempName, compress);
// try {
// FileUtils.moveAtomicReplace(tempName, fileName);
// } catch (DbException e) {
// String newName = fileName + AOStore.SUFFIX_AO_STORE_NEW_FILE;
...
// AOStore source = new AOStore.Builder().storeName(sourceFileName).readOnly().open();
// FileUtils.delete(targetFileName);
// AOStore.Builder b = new AOStore.Builder().storeName(targetFileName);
// if (compress) {
// b.compress();
// }
// AOStore target = b.open();
//
// BTreeStore mapStoreSource = new BTreeStore(sourceFileName, source.getConfig());
// BTreeStore mapStoreTarget = new BTreeStore(targetFileName, target.getConfig());
// compact(mapStoreSource, mapStoreTarget);
// target.close();
// source.close();
// }
//
...
// */
// public static void compact(BTreeStore source, BTreeStore target) {
// BTreeMap<String, String> sourceMeta = source.getMetaMap();
// BTreeMap<String, String> targetMeta = target.getMetaMap();
...
// BTreeMap.Builder<Object, Object> mp = new BTreeMap.Builder<Object, Object>().keyType(new GenericDataType())
// .valueType(new GenericDataType());
// BTreeMap<Object, Object> sourceMap = source.openMap(mapName, mp);
// BTreeMap<Object, Object> targetMap = target.openMap(mapName, mp);
