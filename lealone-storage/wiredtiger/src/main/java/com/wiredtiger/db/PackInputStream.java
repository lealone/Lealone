...
public class PackInputStream {
     ...
     */
    public byte getByte() throws WiredTigerPackingException {
     ...
     */
    public void getByteArray(byte[] dest) throws WiredTigerPackingException {
     ...
     */
    public void getByteArray(byte[] dest, int off, int len) throws WiredTigerPackingException {
     ...
     */
    public byte[] getByteArray() throws WiredTigerPackingException {
     ...
     */
    private int getByteArrayLength() throws WiredTigerPackingException {
        int itemLen = 0;
     ...
     */
    private void getByteArrayInternal(int itemLen, byte[] dest, int off, int destLen) throws WiredTigerPackingException {
        /* TODO: padding. */
        int copyLen = itemLen;
        if (itemLen > destLen) {
            copyLen = destLen;
     ...
     */
    public int getInt() throws WiredTigerPackingException {
     ...
     */
    public long getLong() throws WiredTigerPackingException {
        boolean signed = false;
     ...
     */
    public long getRecord() throws WiredTigerPackingException {
     ...
     */
    public short getShort() throws WiredTigerPackingException {
        boolean signed = false;
     ...
     */
    public String getString() throws WiredTigerPackingException {
        int stringLength = 0;
        int skipnull = 0;
        ...
        if (format.getType() != 'S') {
            ...
            // string length.
            for (; valueOff + stringLength < value.length && value[valueOff + stringLength] != 0; stringLength++) {
            }
            skipnull = 1;
        }
        format.consume();
     ...
     */
    private short unpackShort(boolean signed) throws WiredTigerPackingException {
        long ret = unpackLong(true);
        if ((signed && (ret > Short.MAX_VALUE || ret < Short.MIN_VALUE)) || (!signed && (short) ret < 0)) {
     ...
     * unsigned values, and Java types are all signed.
     */
    private int unpackInt(boolean signed) throws WiredTigerPackingException {
        long ret = unpackLong(true);
        if ((signed && (ret > Integer.MAX_VALUE || ret < Integer.MIN_VALUE)) || (!signed && (int) ret < 0)) {
     ...
     */
    private long unpackLong(boolean signed) throws WiredTigerPackingException {
        int len;
        long unpacked = 0;
        switch (value[valueOff] & 0xf0) {
        case PackUtil.NEG_MULTI_MARKER & 0xff:
            len = PackUtil.SIZEOF_LONG - (value[valueOff++] & 0xf);
        ...
        case (PackUtil.NEG_1BYTE_MARKER | 0x30) & 0xff:
            unpacked = PackUtil.NEG_1BYTE_MIN + PackUtil.GET_BITS(value[valueOff++], 6, 0);
            break;
        ...
        case PackUtil.POS_MULTI_MARKER & 0xff:
            ...
            break;
        default:
            throw new WiredTigerPackingException("Error decoding packed value.");
        }
