...
public class PackFormatInputStream {
     ...
     */
    protected char getType() throws WiredTigerPackingException {
     ...
     */
    protected void checkType(char asking, boolean consume) throws WiredTigerPackingException {

        char expected = getType();
        if (Character.toLowerCase(expected) != Character.toLowerCase(asking))
            throw new WiredTigerPackingException("Format mismatch. Wanted: " + asking + ", got: " + expected);
        if (consume) {
            consume();
        }
    }
    ...
    protected void consume() {
        if (formatRepeatCount > 1) {
            ...
            while (PackUtil.PackSpecialCharacters.indexOf(format.charAt(formatOff)) != -1) {
                ++formatOff;
            }
    ...
    private int getIntFromFormat(boolean advance) {
        int valueLen = 0;
        int countOff;
        for (countOff = 0; Character.isDigit(format.charAt(formatOff + countOff)); countOff++) {
            valueLen *= 10;
