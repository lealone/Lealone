...
class TransactionStatusTable {
    ...
    static void put(AOTransaction transaction, String allLocalTransactionNames) {
    ...
    static boolean validateTransaction(String localTransactionName) {
        if (localTransactionName.startsWith("replication:"))
            return AOTransactionMap.replication.containsKey(localTransactionName.substring("replication:".length()));
        else
