...
import org.lealone.db.index.MVDelegateIndex;
import org.lealone.db.index.MVIndex;
import org.lealone.db.index.MVPrimaryIndex;
import org.lealone.db.index.MVSecondaryIndex;
...
public class MVTable extends TableBase {
    ...
    public void init(Session session) {
        primaryIndex = new MVPrimaryIndex(storageEngine, session, this, getId(), IndexColumn.wrap(getColumns()),
    ...
    private void rebuildIndex(Session session, MVIndex index, String indexName) {
        try {
            if (storageEngine.isInMemory(session.getDatabase())) {
                // in-memory
    ...
    private void rebuildIndexBlockMerge(Session session, MVIndex index) {
        ...
        while (cursor.next()) {
            ...
            if (buffer.size() >= bufferSize) {
                ...
                String mapName = storageEngine.nextTemporaryMapName(session.getDatabase());
