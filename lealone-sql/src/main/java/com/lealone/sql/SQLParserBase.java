...
public class SQLParserBase implements SQLParser {
    ...
    protected StatementBase parseDropSchema() {
        boolean ifExists = readIfExists(false);
    ...
    protected Expression parseVariable() {
        read();
        Expression r = new Variable(session, readAliasIdentifier());
        if (readIf(":=")) {
    ...
    protected String readStringOrIdentifier() {
    ...
    protected Column parseColumnWithType(String columnName) {
        ...
        if (readIf("BINARY")) {
            if (dataType.type == Value.STRING) {
                dataType = DataType.getTypeByName("VARCHAR");
            }
        }
        // MySQL compatibility
        readIf("UNSIGNED");
    ...
    protected StatementBase parseSet() {
        if (readIf("@")) { // session变量
            ...
            StatementBase other = parseSetOther();
            if (other != null)
                return other;
            if (!identifiersToUpper)
