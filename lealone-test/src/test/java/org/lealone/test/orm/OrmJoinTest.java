...
public class OrmJoinTest extends OrmTestBase {
    ...
    void join() {
        ...
        customerList = c.select(c.id, c.name, c.phone, o.orderId, o.orderDate, o.customerId).join(o).on().id
        ...
        customerList = c.join(o).on().id.eq(o.customerId).where().id.eq(100).or().m(o).customerId.eq(200).m(c)
                .findList();
        assertEquals(2, customerList.size());
