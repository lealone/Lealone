...
public class JDBCExample {
    static Connection getConnection() throws Exception {
        String url = "jdbc:lealone:tcp://localhost:5210/mydb?default_table_engine=MySQL";
        url = "jdbc:lealone:tcp://localhost:5210/mydb";
        //url = "jdbc:lealone:embed:mydb?default_table_engine=MySQL";
    ...
    static void crud() throws Exception {
        ...
        //stmt.executeUpdate("CREATE TABLE IF NOT EXISTS test (f1 int primary key, f2 long) engine MySQL"); 
        stmt.executeUpdate("CREATE TABLE IF NOT EXISTS test (f1 int primary key, f2 long)");
        //stmt.executeUpdate("CREATE memory TABLE IF NOT EXISTS test2 (f1 int primary key, f2 long)");
        stmt.executeUpdate("CREATE TABLE IF NOT EXISTS test3 (f1 int primary key, f2 long)"); //MySQL
