...
public class Gossiper implements IFailureDetectionEventListener, GossiperMBean {
        ...
        public void run() {
            try {
                ...
                if (gDigests.size() > 0) {
                    ...
                    P2pPacketOut<GossipDigestSyn> message = new P2pPacketOut<>(digestSynMessage);
    ...
    private boolean doGossipToLiveMember(P2pPacketOut<GossipDigestSyn> message) {
    ...
    private void doGossipToUnreachableMember(P2pPacketOut<GossipDigestSyn> message) {
    ...
    private void doGossipToSeed(P2pPacketOut<GossipDigestSyn> prod) {
        int size = seeds.size();
        if (size > 0) {
     ...
     */
    private boolean sendGossip(P2pPacketOut<GossipDigestSyn> message, Set<NetNode> epSet) {
     ...
     * @return
     */
    public int getMaxNodeStateVersion(NodeState epState) {
    ...
    public NodeState getStateForVersionBiggerThan(NetNode forNode, int version) {
    ...
    public void notifyFailureDetector(Map<NetNode, NodeState> remoteEpStateMap) {
    ...
    private void markAlive(final NetNode addr, final NodeState localState) {
        ...
        P2pPacketOut<EchoPacket> echoMessage = new P2pPacketOut<>(new EchoPacket());
        logger.trace("Sending a EchoMessage to {}", addr);
        IAsyncCallback<GossipResponse> echoHandler = new IAsyncCallback<GossipResponse>() {
            @Override
            ...
            @Override
            public void response(P2pPacketIn<GossipResponse> msg) {
    ...
    public void applyStateLocally(Map<NetNode, NodeState> epStateMap) {
    ...
    */
    ...
    public void stop() {
        ...
        P2pPacketOut<?> message = new P2pPacketOut<>(new GossipShutdown());
